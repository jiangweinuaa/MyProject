...
public class ytoHttpClientUtil {
  ...
  public static String postRequest(String uri, String jsonParams) {
      ...
      try {
    	  HelpTools.writelog_fileName("【调用物流ytoHttpClientUtil】发送url:"+uri+",请求req:"+jsonParams,logFileName);
    	  ...
    	  try{
        	  HelpTools.writelog_fileName("【调用物流ytoHttpClientUtil】返回异常:"+e,logFileName);
          }catch (Exception ex) {
          }
      ...
      try{
    	  HelpTools.writelog_fileName("【调用物流ytoHttpClientUtil】返回res:"+output,logFileName);
      }catch (Exception e) {
      }
      return output;
  }
