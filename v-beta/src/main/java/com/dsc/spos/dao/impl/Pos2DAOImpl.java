...
@Repository("Pos2Dao")
@Transactional("pos2")
public class Pos2DAOImpl implements DsmDAO
{

    private Logger loger = LogManager.getLogger(Pos2DAOImpl.class.getName());
    private static HibernateTemplate hibernateTemplate = new HibernateTemplate();

    @SuppressWarnings("static-access")
    @Autowired
    @Required
    public void setSessionFactory(@Qualifier("pos2") SessionFactory sessionFactory) {
    ...
    {
        final Pos2DAOImpl.Result r = new Pos2DAOImpl.Result();
        ...
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work()
        {
            ...
            public void execute(Connection conn) throws SQLException
            {
                ...
                try
                {
                    result=Pos2DAOImpl.this.GetNativeSQL(conn, parseSql);
                    r.setDatas(result);
                    result=null;
                }
                catch(Exception e)
    ...
    public boolean doDelete(Connection conn, String tableName, Map<String, DataValue> conditions) throws Exception
    {
        ...
        try
        {
            ...
            for (String key : keys)
            {
                ...
                Pos2DAOImpl.this.getExpression(key, v, sb);
            }
    ...
    public boolean doDelete(final String tableName, final Map<String, DataValue> conditions) throws Exception {
        ...
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work() {
            ...
            public void execute(Connection conn) throws SQLException {
                boolean b;
                try {
                    b = Pos2DAOImpl.this.doDelete(conn, tableName, conditions);
                    r.setResult(b);
    ...
    public int exec(final String tableName) throws Exception {
        ...
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work(){
            ...
            public void execute(Connection conn) throws SQLException {
                try {
                    int rows = Pos2DAOImpl.this.exec(conn, tableName);
                    r.setRows(rows);
    ...
    public int insert(final String tableName, final String[] columns, final DataValue[] values) throws Exception {
        ...
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work(){
            ...
            public void execute(Connection conn) throws SQLException {
                try {
                    int rows = Pos2DAOImpl.this.insert(conn, tableName, columns, values);
                    r.setRows(rows);
                    r.setResult(Boolean.TRUE);
                } catch (SQLException e) {
                    r.setResult(Boolean.FALSE);
    ...
    public boolean update(final String tableName, final Map<String, DataValue> values, final Map<String, DataValue> conditions) throws Exception {
        ...
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work(){
            ...
            public void execute(Connection conn) throws SQLException {
                try {
                    boolean b = Pos2DAOImpl.this.update(conn, tableName, values, conditions);
                    r.setResult(b);
                } catch (SQLException e) {
                    r.setResult(Boolean.FALSE);
                    throw e;
                    ...
                    for (DataProcessBean dpb : data)
                    {
                        switch(dpb.getProcessType())
                        {
                            case INSERT:
                                ...
                                {
                                    Pos2DAOImpl.this.insert(conn, dpb.getTableName(), dpb.getColumns(), row);
                                }
                                break;
                            case UPDATE:
                                Pos2DAOImpl.this.update(conn, dpb.getTableName(), dpb.getValues(), dpb.getConditions());
                                break;
                            case DELETE:
                                Pos2DAOImpl.this.doDelete(conn, dpb.getTableName(), dpb.getConditions());
                                break;
                            case EXEC:
                                Pos2DAOImpl.this.exec(conn, dpb.getTableName());
                                break;
                            case PROCEDURE:
                                Pos2DAOImpl.this.storedProcedureProcess(conn,dpb.getProcedure(),dpb.getInputParameter());
                                break;
    ...
    public Map<Integer,Object> storedProcedure (final String procedure, final Map<Integer,Object> inputParameter,final Map<Integer,Integer> outParameter) throws Exception{
        final Pos2DAOImpl.storedProcedureResult r = new Pos2DAOImpl.storedProcedureResult();
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work(){
            @Override
            public void execute(Connection conn) throws SQLException
            {
    ...
    public List<Map<String, Object>> executeQuerySQL_BindSQL(String sql, List<DataValue> values) throws Exception
    {
        ...
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work()
        {
            ...
            public void execute(Connection conn) throws SQLException
            {
                ...
                try
                {
                    result=Pos2DAOImpl.this.GetNativeSQL_BindSQL(conn,sql, values);
                    r.setDatas(result);
                    result=null;
                }
                catch(Exception e)
