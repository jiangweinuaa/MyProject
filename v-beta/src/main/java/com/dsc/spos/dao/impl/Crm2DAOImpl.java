...
@Repository("Crm2Dao")
@Transactional("crm2")
public class Crm2DAOImpl implements DsmDAO
{

    private Logger loger = LogManager.getLogger(Crm2DAOImpl.class.getName());
    private static HibernateTemplate hibernateTemplate = new HibernateTemplate();

    @SuppressWarnings("static-access")
    @Autowired
    @Required
    public void setSessionFactory(@Qualifier("crm2") SessionFactory sessionFactory) {
    ...
    {
        final Crm2DAOImpl.Result r = new Crm2DAOImpl.Result();
        ...
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work()
        {
            ...
            public void execute(Connection conn) throws SQLException
            {
                ...
                try
                {
                    result=Crm2DAOImpl.this.GetNativeSQL(conn, parseSql);
                    r.setDatas(result);
                    result=null;
                }
                catch(Exception e)
    ...
    private String parseSql(String sql, String[] values, boolean isShowSql)
    {
        ...
        if (values != null)
        {
            ...
            {
                otcs[i] = values[i].replaceAll("--", ""); //預防 sql injection 的攻擊
            }
    ...
    public boolean doDelete(Connection conn, String tableName, Map<String, DataValue> conditions) throws Exception
    {
        ...
        try
        {
            ...
            for (String key : keys)
            {
                ...
                Crm2DAOImpl.this.getExpression(key, v, sb);
            }
    ...
    public boolean doDelete(final String tableName, final Map<String, DataValue> conditions) throws Exception {
        ...
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work() {
            ...
            public void execute(Connection conn) throws SQLException {
                boolean b;
                try {
                    b = Crm2DAOImpl.this.doDelete(conn, tableName, conditions);
                    r.setResult(b);
    ...
    public int exec(final String tableName) throws Exception {
        ...
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work(){
            ...
            public void execute(Connection conn) throws SQLException {
                try {
                    int rows = Crm2DAOImpl.this.exec(conn, tableName);
                    r.setRows(rows);
    ...
    public int insert(final String tableName, final String[] columns, final DataValue[] values) throws Exception {
        ...
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work(){
            ...
            public void execute(Connection conn) throws SQLException {
                try {
                    int rows = Crm2DAOImpl.this.insert(conn, tableName, columns, values);
                    r.setRows(rows);
                    r.setResult(Boolean.TRUE);
                } catch (SQLException e) {
                    r.setResult(Boolean.FALSE);
    ...
    public boolean update(final String tableName, final Map<String, DataValue> values, final Map<String, DataValue> conditions) throws Exception {
        ...
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work(){
            ...
            public void execute(Connection conn) throws SQLException {
                try {
                    boolean b = Crm2DAOImpl.this.update(conn, tableName, values, conditions);
                    r.setResult(b);
                } catch (SQLException e) {
                    r.setResult(Boolean.FALSE);
                    throw e;
                    ...
                    for (DataProcessBean dpb : data)
                    {
                        switch(dpb.getProcessType())
                        {
                            case INSERT:
                                ...
                                {
                                    Crm2DAOImpl.this.insert(conn, dpb.getTableName(), dpb.getColumns(), row);
                                }
                                break;
                            case UPDATE:
                                Crm2DAOImpl.this.update(conn, dpb.getTableName(), dpb.getValues(), dpb.getConditions());
                                break;
                            case DELETE:
                                Crm2DAOImpl.this.doDelete(conn, dpb.getTableName(), dpb.getConditions());
                                break;
                            case EXEC:
                                Crm2DAOImpl.this.exec(conn, dpb.getTableName());
                                break;
                            case PROCEDURE:
                                Crm2DAOImpl.this.storedProcedureProcess(conn,dpb.getProcedure(),dpb.getInputParameter());
                                break;
    ...
    public Map<Integer,Object> storedProcedure (final String procedure, final Map<Integer,Object> inputParameter,final Map<Integer,Integer> outParameter) throws Exception{
        final Crm2DAOImpl.storedProcedureResult r = new Crm2DAOImpl.storedProcedureResult();
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work(){
            @Override
            public void execute(Connection conn) throws SQLException
            {
    ...
    public List<Map<String, Object>> executeQuerySQL_BindSQL(String sql, List<DataValue> values) throws Exception
    {
        ...
        hibernateTemplate.getSessionFactory().getCurrentSession().doWork(new Work()
        {
            ...
            public void execute(Connection conn) throws SQLException
            {
                ...
                try
                {
                    result=Crm2DAOImpl.this.GetNativeSQL_BindSQL(conn,sql, values);
                    r.setDatas(result);
                    result=null;
                }
                catch(Exception e)
