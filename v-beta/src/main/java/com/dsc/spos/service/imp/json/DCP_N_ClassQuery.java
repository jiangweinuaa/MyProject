...
import com.dsc.spos.json.cust.req.DCP_N_ClassQueryReq;
import com.dsc.spos.json.cust.res.DCP_N_ClassQueryRes;
import com.dsc.spos.json.cust.res.DCP_N_ClassQueryRes.*;
 ...
 * @since  2024-04-18
 */
public class DCP_N_ClassQuery extends SPosBasicService<DCP_N_ClassQueryReq, DCP_N_ClassQueryRes> {
    @Override
    protected boolean isVerifyFail(DCP_N_ClassQueryReq req) throws Exception {
    ...
    @Override
    protected TypeToken<DCP_N_ClassQueryReq> getRequestType() {
        return new TypeToken<DCP_N_ClassQueryReq>(){};
    }

    @Override
    protected DCP_N_ClassQueryRes getResponseType() {
        return new DCP_N_ClassQueryRes();
    }

    @Override
    protected DCP_N_ClassQueryRes processJson(DCP_N_ClassQueryReq req) throws Exception {

        DCP_N_ClassQueryRes res = this.getResponse();
        ...
        try {
            ...
            if (getQDataDetail != null && !getQDataDetail.isEmpty()) {
                ...
                for (Map<String, Object> map : catDatas) {
                    DCP_N_ClassQueryRes.Datas oneLv1 = res.new Datas();
                    ...
                    if (levelId.isEmpty() || upClassNo.isEmpty() || !levelId.equals("2") || isKeyTxt) {
                        ...
                        oneLv1.setIsPublic(map.get("ISPUBLIC").toString());  //分类性质：1共用分组2.指定分组
                        ...
                        oneLv1.setSubClassList(new ArrayList<>());
                        /*
    ...
    @Override
    protected String getQuerySql(DCP_N_ClassQueryReq req) throws Exception {
        ...
        sqlBuff.append("SELECT DISTINCT a.CLASSTYPE,a.CLASSNO,b.LANG_TYPE,b.CLASSNAME,b.DISPLAYNAME,a.LEVELID,a.UPCLASSNO,UL.CLASSNAME as UPCLASSNAME,UL.DISPLAYNAME AS UPDISPLAYNAME,a.SORTID,a.MEMO,a.STATUS,a.ispublic,"
        ...
        if (!Check.Null(isPublic)){
            sqlBuff.append(" and a.ispublic='" + isPublic + "' ");
        }
...
//		}
    ...
    private void setChildrenDatas(DCP_N_ClassQueryReq req, Datas oneLv2, List<Map<String, Object>> allDatas) {
        String curLangType = req.getLangType();
        if (curLangType == null || curLangType.isEmpty()) {
            curLangType = "zh_CN";
        }
        ...
        try {
            ...
            if (!nextDatas.isEmpty()) {
                String ISHTTPS= PosPub.getPARA_SMS(dao, req.geteId(), "", "ISHTTPS");
                String httpStr=ISHTTPS.equals("1")?"https://":"http://";

                String DomainName=PosPub.getPARA_SMS(dao, req.geteId(), "", "DomainName");
                for (Map<String, Object> datas : nextDatas) {
                    Datas oneLv1 = new DCP_N_ClassQueryRes().new Datas();
                    ...
                    oneLv1.setIsPublic(datas.get("ISPUBLIC").toString());  //分类性质：1共用分组2.指定分组

                    //特殊处理，前端传入空的，后端数据库存-1，此处还要返回给前端空，这个会员合并代码复制原来的，又加上新的内容，表字段亦有废除亦有添加全混在一起，最后肯定是一锅粥
                    String remindType = "";
                    if (!"-1".equals(datas.get("REMINDTYPE").toString())){
                        remindType = datas.get("REMINDTYPE").toString();
                    }
                    oneLv1.setRemindType(remindType);
     ...
     * 获取下一层级信息
     */
    private List<Map<String, Object>> getChildDatas(List<Map<String, Object>> allDatas, String classType, String classNo) {
