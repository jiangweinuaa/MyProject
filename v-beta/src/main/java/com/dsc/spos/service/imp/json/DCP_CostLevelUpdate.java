...
import com.dsc.spos.json.cust.req.DCP_CostLevelUpdateReq;
import com.dsc.spos.json.cust.res.DCP_CostLevelUpdateRes;
...
public class DCP_CostLevelUpdate extends SPosAdvanceService<DCP_CostLevelUpdateReq, DCP_CostLevelUpdateRes> {

    @Override
    protected void processDUID(DCP_CostLevelUpdateReq req, DCP_CostLevelUpdateRes res) throws Exception {
        ...
        try {
            ...
            if (null==repeat || repeat.isEmpty()){
                throw new DispatchService.SPosCodeException(DispatchService.SPosCodeException.CODE_EXCEPTION_TYPE.E400,req.getRequest().getCostGroupingId()+"不存在");
            }
    ...
    @Override
    protected List<InsBean> prepareInsertData(DCP_CostLevelUpdateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected List<UptBean> prepareUpdateData(DCP_CostLevelUpdateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected List<DelBean> prepareDeleteData(DCP_CostLevelUpdateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected boolean isVerifyFail(DCP_CostLevelUpdateReq req) throws Exception {
    ...
    @Override
    protected TypeToken<DCP_CostLevelUpdateReq> getRequestType() {
        return new TypeToken<DCP_CostLevelUpdateReq>() {
        };
    }

    @Override
    protected DCP_CostLevelUpdateRes getResponseType() {
        return new DCP_CostLevelUpdateRes();
    }
}
