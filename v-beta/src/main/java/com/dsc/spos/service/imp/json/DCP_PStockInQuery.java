...
import com.dsc.spos.json.cust.req.DCP_PStockInQueryReq;
import com.dsc.spos.json.cust.res.DCP_PStockInQueryRes;
...
public class DCP_PStockInQuery extends SPosBasicService<DCP_PStockInQueryReq,DCP_PStockInQueryRes> {
	@Override
	protected boolean isVerifyFail(DCP_PStockInQueryReq req) throws Exception {
	...
	@Override
	protected TypeToken<DCP_PStockInQueryReq> getRequestType() {
		return new TypeToken<DCP_PStockInQueryReq>(){};
	}
	
	@Override
	protected DCP_PStockInQueryRes getResponseType() {
		return new DCP_PStockInQueryRes();
	}
	
	@Override
	protected DCP_PStockInQueryRes processJson(DCP_PStockInQueryReq req) throws Exception {
		//2018-08-07添加docType;
		...
		//try {
			...
			//查询资料
			DCP_PStockInQueryRes res = this.getResponse();
			//单头单身查询
			...
			if (getQData != null && !getQData.isEmpty()) {
				...
				for (Map<String, Object> oneData : getQHeader) {
					DCP_PStockInQueryRes.level1Elm oneLv1 = res.new level1Elm();
	...
	@Override
	protected String getQuerySql(DCP_PStockInQueryReq req) throws Exception {
        ...
        String productMode = PosPub.getPARA_SMS(dao, req.geteId(), "", "ProductMode");
		...
		sqlbuf.append(" "
				...
				+ " from ( " );
		if (docType.equals("0")&&"0".equals(productMode)) {
			sqlbuf.append( " select 1 as rw,processtaskno as dno,cast(a.status as int) as status,bdate,a.eid,a.shopid from dcp_processtask a"
					+ " left join (select ofno from dcp_pstockin where eid='"+eId+"' and shopid='"+shopId+"' and ofno is not null and status='0') dd on a.processtaskno = dd.ofno"
					+ " left join dcp_ptemplate c on a.eid=c.eid and a.ptemplateno=c.ptemplateno and c.doc_type='2'"
					+ " where a.eid='"+ eId +"' and a.shopid='"+ shopId +"'  and a.pdate between "+beginDate +" and "+endDate+" and dd.ofno is null ");//and a.pdate = '"+ sDate +"'
			...
			if (keyTxt != null && keyTxt.length() > 0) {
				sqlbuf.append(" and (a.tot_amt like '%%"+keyTxt+"%%' or a.tot_pqty like '%%"+keyTxt+"%%'"
						+ " or a.processtaskno like '%%"+keyTxt+"%%' or a.memo like '%%"+keyTxt+"%%'"
						+ " or a.ptemplateno like '%%"+keyTxt+"%%' or c.ptemplate_name like '%%"+keyTxt+"%%')");
			}
		...
		sqlbuf.append(" "
				...
				+ " left join dcp_ptemplate c on a.eid=c.eid and a.ptemplateno=c.ptemplateno and c.doc_type='2'"
				+ " where a.eid='"+eId+"' and a.shopid='"+shopId+"' and a.bdate between "+beginDate +" and "+endDate+" and a.doc_type='"+docType+"'");
		if (status != null && status.length()!=0) {
			sqlbuf.append(" and a.status = '"+ status +"'  ");
		}
		if (keyTxt != null && keyTxt.length() > 0) {
		...
		sqlbuf.append(""
				...
				+ " f1.name as modifybyname,f2.name as cancelbyname,f3.name as confirmbyname,f4.name as submitbyname,"
				+ " f5.name as accountbyname,e1.name as employeename,d1.departname,d2.departname as createdeptname  "
		...
		if( docType.equals("0")&&"0".equals(productMode)) {
			sqlbuf.append( " SELECT distinct 1 as rw,"
					...
					+ " cast('' as nvarchar2(30)) as PSTOCKINNO,A.SHOPID,a.bdate,A.memo,cast(A.status as int) as status,cast('' as nvarchar2(1)) as oType "
					...
					+ " A.PROCESSPLANNO,A.TASK0NO,A.DTNO,dt1.dtname,dt1.begin_time,dt1.end_time,CAST('' AS nvarchar2(30)) as employeeid,CAST('' AS nvarchar2(30)) as departid,CAST('' AS nvarchar2(30)) as CREATEDEPTID,A.PRODTYPE,CAST('' AS nvarchar2(30)) as OOFNO,CAST('' AS nvarchar2(30)) as OOTYPE "
					+ " FROM DCP_PROCESSTASK A "
					+ " INNER JOIN DCP_PROCESSTASK_DETAIL b ON A.EID=b.EID AND A.SHOPID=b.SHOPID AND A.PROCESSTASKNO=b.PROCESSTASKNO and A.BDATE=b.BDATE and nvl(b.DISPATCHSTATUS,'0')='0' and b.GOODSSTATUS in ('0','1') "
                    + " left join DCP_DINNERTIME dt1 on A.eid = dt1.eid and A.organizationno = dt1.shopid and A.dtno = dt1.dtno "
					+ " where a.eid='"+ eId +"'  and a.shopid='"+ shopId +"' and a.pdate between "+beginDate +" and "+endDate+" "//and a.pdate = '"+ sDate +"'
			);
			
			if (status != null && status.length()!=0){
				sqlbuf.append(" AND A.status = '"+ status +"' ");
			}
			//生产入库需要多次入库  所以这里先去掉此状态  增加过来生产日期是当天的就能选到
			else{
				//sqlbuf.append(" AND (A.status = '6' or a.status='7' or a.status = '0' or a.status = '2')  ");
				sqlbuf.append(" AND (A.status = '6')  ");
			}
			sqlbuf.append(" union all ");
		}
		...
		sqlbuf.append(" to_number(a.bDate) bdate,A.memo,A.status, A.oType  "
                ...
                + " dt1.dtname,dt1.begin_time,dt1.end_time,a.employeeid,a.departid,a.CREATEDEPTID,A.PRODTYPE,A.OOFNO,A.OOTYPE "
				+ " FROM DCP_PSTOCKIN A  "
				+ " left join DCP_DINNERTIME dt1 on A.eid=dt1.eid and A.organizationno=dt1.shopid and A.dtno=dt1.dtno "
				+ " "
		);
