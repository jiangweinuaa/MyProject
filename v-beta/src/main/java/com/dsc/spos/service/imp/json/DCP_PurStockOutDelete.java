...
import com.dsc.spos.json.cust.req.DCP_PurStockOutDeleteReq;
import com.dsc.spos.json.cust.res.DCP_PurOrderDeleteRes;
import com.dsc.spos.json.cust.res.DCP_PurStockOutDeleteRes;
...
public class DCP_PurStockOutDelete extends SPosAdvanceService<DCP_PurStockOutDeleteReq, DCP_PurStockOutDeleteRes> {

    @Override
    protected boolean isVerifyFail(DCP_PurStockOutDeleteReq req) throws Exception {
     ...
     */
    @Override
    protected String getQuerySql(DCP_PurStockOutDeleteReq req) throws Exception {
        // TODO Auto-generated method stub
        return "";
    }

    private boolean CheckPurStockOutOrder(String eid,String organizationno,String no, String billType) throws Exception {
        StringBuilder sb=new StringBuilder();
        sb.append("select * from DCP_PURSTOCKIN a" +
                " where a.PSTOCKINNO='"+no+"' and a.status='0' and a.eid='"+eid+"' and a.organizationno='"+organizationno+"' and a.billTYPE='"+billType+"' ");
    ...
    @Override
    protected void processDUID(DCP_PurStockOutDeleteReq req, DCP_PurStockOutDeleteRes res) throws Exception {
        // TODO Auto-generated method stub
        String sql = "";
        try
        {
            String no = req.getRequest().getPStockOutNo();
            String billType = req.getRequest().getBillType();
            String eId = req.geteId();
            String organizationNO = req.getOrganizationNO();

            if(!CheckPurStockOutOrder(eId,organizationNO,no,billType)){
            ...
            DelBean db1 = new DelBean("DCP_PURSTOCKIN");
            db1.addCondition("PSTOCKINNO", new DataValue(no, Types.VARCHAR));
            db1.addCondition("BILLTYPE",new DataValue(billType,Types.VARCHAR));
            db1.addCondition("EID", new DataValue(eId, Types.VARCHAR));
            db1.addCondition("ORGANIZATIONNO", new DataValue(organizationNO, Types.VARCHAR));
            this.addProcessData(new DataProcessBean(db1));

            DelBean db2 = new DelBean("DCP_PURSTOCKIN_DETAIL");
            db2.addCondition("PSTOCKINNO", new DataValue(no, Types.VARCHAR));
            db2.addCondition("EID", new DataValue(eId, Types.VARCHAR));
            db2.addCondition("ORGANIZATIONNO", new DataValue(organizationNO, Types.VARCHAR));
            this.addProcessData(new DataProcessBean(db2));

            DelBean db3 = new DelBean("DCP_PURSTOCKIN_LOTS");
            db3.addCondition("PSTOCKINNO", new DataValue(no, Types.VARCHAR));
    ...
    @Override
    protected List<InsBean> prepareInsertData(DCP_PurStockOutDeleteReq req) throws Exception {
        // TODO Auto-generated method stub
        return null;
    }

    @Override
    protected List<UptBean> prepareUpdateData(DCP_PurStockOutDeleteReq req) throws Exception {
        // TODO Auto-generated method stub
        return null;
    }

    @Override
    protected List<DelBean> prepareDeleteData(DCP_PurStockOutDeleteReq req) throws Exception {
        // TODO Auto-generated method stub
        return null;
    }
    @Override
    protected TypeToken<DCP_PurStockOutDeleteReq> getRequestType() {
        // TODO Auto-generated method stub
        return new TypeToken<DCP_PurStockOutDeleteReq>(){};
    }
    @Override
    protected DCP_PurStockOutDeleteRes getResponseType() {
        // TODO Auto-generated method stub
        return new DCP_PurStockOutDeleteRes();
    }

    private boolean canDeleteDoc(DCP_PurStockOutDeleteReq req,String modularNo,String range ) throws Exception{
        ...
        if(opENos.size()>0){
            ...
            String purOrderSql="with p AS ( " + withasSql_employeeno + ") " +
                    " select * from DCP_PURSTOCKIN a " +
                    " inner join p on p.EMPLOYEENO=a.OWNOPID " +
                    " where a.PSTOCKINNO='"+orderNo+"' and a.BILLTYPE='"+billType+"' "
                    ;
