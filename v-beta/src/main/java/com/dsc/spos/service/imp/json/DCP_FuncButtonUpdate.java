...
import com.dsc.spos.json.cust.req.DCP_FuncButtonUpdateReq;
import com.dsc.spos.json.cust.req.DCP_FuncButtonUpdateReq.level1Elm;
import com.dsc.spos.json.cust.req.DCP_FuncButtonUpdateReq.level1shopsElm;
import com.dsc.spos.json.cust.res.DCP_FuncButtonUpdateRes;
...
/**
 * 服務函數：FuncButtonUpdateDCP
 * 服务说明：功能按键修改
 * @author jinzma 
 * @since  2019-02-20
 */
public class DCP_FuncButtonUpdate extends SPosAdvanceService<DCP_FuncButtonUpdateReq,DCP_FuncButtonUpdateRes >{

	@Override
	protected void processDUID(DCP_FuncButtonUpdateReq req, DCP_FuncButtonUpdateRes res) throws Exception {
		// TODO 自动生成的方法存根
		...
		try 
		{
			...
			{
				//删除单身
				DelBean db = new DelBean("DCP_FUNCBUTTON");
				db.addCondition("EID", new DataValue(eId, Types.VARCHAR));
				db.addCondition("PTEMPLATENO", new DataValue(ptemplateNO, Types.VARCHAR));
				this.addProcessData(new DataProcessBean(db));

				//删除门店
				DelBean db_shop = new DelBean("DCP_FUNCBUTTON_SHOP");
				db_shop.addCondition("EID", new DataValue(eId, Types.VARCHAR));
				db_shop.addCondition("PTEMPLATENO", new DataValue(ptemplateNO, Types.VARCHAR));
	...
	@Override
	protected List<InsBean> prepareInsertData(DCP_FuncButtonUpdateReq req) throws Exception {
		// TODO 自动生成的方法存根
		return null;
	}

	@Override
	protected List<UptBean> prepareUpdateData(DCP_FuncButtonUpdateReq req) throws Exception {
		// TODO 自动生成的方法存根
		return null;
	}

	@Override
	protected List<DelBean> prepareDeleteData(DCP_FuncButtonUpdateReq req) throws Exception {
		// TODO 自动生成的方法存根
		return null;
	}

	@Override
	protected boolean isVerifyFail(DCP_FuncButtonUpdateReq req) throws Exception {
		// TODO 自动生成的方法存根
	...
	@Override
	protected TypeToken<DCP_FuncButtonUpdateReq> getRequestType() {
		// TODO 自动生成的方法存根
		return new TypeToken<DCP_FuncButtonUpdateReq>(){};
	}

	@Override
	protected DCP_FuncButtonUpdateRes getResponseType() {
		// TODO 自动生成的方法存根
		return new DCP_FuncButtonUpdateRes(); 
	}

	private boolean checkExist(DCP_FuncButtonUpdateReq req)  throws Exception {
		String sql = null;
		boolean exist = false;
		String eId = req.geteId();
		String ptemplateNO = req.getRequest().getPtemplateNo();
