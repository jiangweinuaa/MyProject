...
import com.dsc.spos.json.cust.req.DCP_QuotationCreateReq;
import com.dsc.spos.json.cust.req.DCP_QuotationCreateReq.level2Elm;
import com.dsc.spos.json.cust.res.DCP_QuotationCreateRes;
 ...
 *
 */
public class DCP_QuotationCreate extends SPosAdvanceService<DCP_QuotationCreateReq, DCP_QuotationCreateRes> {

	@Override
	protected void processDUID(DCP_QuotationCreateReq req, DCP_QuotationCreateRes res) throws Exception {
		...
		try {
			...
			String quotationRecordNo = this.getMaxBillNo(req);
			...
			String[] columns_hm ={"EID","QUOTATIONRECORDNO","SHOPID","TELEPHONE","SHOPEMAIL","ADDRESS",
					"CUSTOMNAME" , "MOBILE" , "CUSTOMEMAIL" , "REFEREES","REMARK","TOT_AMT", "OPNO","STATUS",
					"LASTMODITIME","CREATETIME"};
			...
			DataValue[] insValue_hm = new DataValue[] 
					{
							...
							new DataValue(shopEmail ,Types.VARCHAR),
							new DataValue(address, Types.VARCHAR),
							new DataValue(customName, Types.VARCHAR),
							new DataValue(mobile , Types.VARCHAR),
							new DataValue(customEmail, Types.VARCHAR),
							new DataValue(referees, Types.VARCHAR),
			...
			InsBean ib_hm = new InsBean("DCP_QUOTATIONRECORD", columns_hm);
	...
	@Override
	protected List<InsBean> prepareInsertData(DCP_QuotationCreateReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	protected List<UptBean> prepareUpdateData(DCP_QuotationCreateReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	protected List<DelBean> prepareDeleteData(DCP_QuotationCreateReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	protected boolean isVerifyFail(DCP_QuotationCreateReq req) throws Exception {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	protected TypeToken<DCP_QuotationCreateReq> getRequestType() {
		// TODO Auto-generated method stub
		return new TypeToken<DCP_QuotationCreateReq>(){};
	}

	@Override
	protected DCP_QuotationCreateRes getResponseType() {
		// TODO Auto-generated method stub
		return new DCP_QuotationCreateRes();
	}
	
	private String getMaxBillNo(DCP_QuotationCreateReq req) throws Exception{
		String billNo = "";
		...
		billNo = "BJDH" + matter.format(dt);
		sqlbuf.append("" + "select billNO  from ( " + "select max(quotationRecordNo) as  billNO "
				+ "  from DCP_quotationRecord " + " where eId='"+req.geteId()+"'  " 
				+ " and quotationRecordNo like '%%" + billNo + "%%' "); 
		sqlbuf.append(" ) TBL ");
		sql = sqlbuf.toString();
		...
		if (getQData != null && getQData.isEmpty() == false) {
			...
			if (billNo != null && billNo.length() > 0) {
				...
				billNo = "BJDH" + matter.format(dt) + "00001";
			}
		} else {
			billNo = "BJDH" + matter.format(dt) + "00001";
		}
