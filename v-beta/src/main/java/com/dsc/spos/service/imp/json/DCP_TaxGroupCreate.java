...
import com.dsc.spos.json.cust.req.DCP_TaxGroupCreateReq;
import com.dsc.spos.json.cust.res.DCP_TaxGroupCreateRes;
...
public class DCP_TaxGroupCreate extends SPosAdvanceService<DCP_TaxGroupCreateReq, DCP_TaxGroupCreateRes> {

    @Override
    protected void processDUID(DCP_TaxGroupCreateReq req, DCP_TaxGroupCreateRes res) throws Exception {
        ...
        try {
            ...
            List<Map<String,Object>> isRepeat = this.doQueryData(String.format("SELECT * FROM dcp_taxGroup WHERE eid='%s' and TAXGROUPNO='%s' ",req.geteId(),req.getRequest().getTaxGroupNo()),null);
            if (isRepeat != null && !isRepeat.isEmpty()) {
                res.setSuccess(false);
                res.setServiceStatus("200");
                res.setServiceDescription(req.getRequest().getTaxGroupNo()+"已存在!");
                return;
            }

            ColumnDataValue dcp_taxGroup = new ColumnDataValue();

            dcp_taxGroup.add("EID", DataValues.newString(req.geteId()));
            dcp_taxGroup.add("TAXGROUPNO", DataValues.newString(req.getRequest().getTaxGroupNo()));
            ...
            dcp_taxGroup.add("CREATEOPID", DataValues.newString(req.getEmployeeNo()));
            dcp_taxGroup.add("CREATEDEPTID", DataValues.newString(req.getDepartmentNo()));
            dcp_taxGroup.add("CREATETIME", DataValues.newDate(DateFormatUtils.getNowDateTime()));
            ...
            addProcessData(new DataProcessBean(DataBeans.getInsBean("DCP_TAXGROUP", dcp_taxGroup)));

            for (DCP_TaxGroupCreateReq.GoodsList goods : req.getRequest().getGoodsList()) {
                ...
                addProcessData(new DataProcessBean(DataBeans.getInsBean("DCP_TAXGROUP_DETAIL", dcp_taxGroup_detail)));
            }
     ...
     */
    private boolean verifyGoodsList(DCP_TaxGroupCreateReq req) throws Exception {
...
//            枚举: 1-品类 2-商品
        ...
        if (check != null && !check.isEmpty()) {
            for (Map<String, Object> map : check) {
                if ("1".equals(map.get("ATTRTYPE"))) {
                    errorMsg.append("品类").append(map.get("ATTRID")).append("已设置税分类，请检查！").append(map.get("TAXGROUPNO"));
                } else {
                    errorMsg.append("商品").append(map.get("ATTRID")).append("已设置税分类，请检查！").append(map.get("TAXGROUPNO"));
                }
    ...
    @Override
    protected List<InsBean> prepareInsertData(DCP_TaxGroupCreateReq req) throws Exception {
        return null;
    }

    @Override
    protected List<UptBean> prepareUpdateData(DCP_TaxGroupCreateReq req) throws Exception {
        return null;
    }

    @Override
    protected List<DelBean> prepareDeleteData(DCP_TaxGroupCreateReq req) throws Exception {
        return null;
    }

    @Override
    protected boolean isVerifyFail(DCP_TaxGroupCreateReq req) throws Exception {
        return false;
    }

    @Override
    protected TypeToken<DCP_TaxGroupCreateReq> getRequestType() {
        return new TypeToken<DCP_TaxGroupCreateReq>() {

        };
    }

    @Override
    protected DCP_TaxGroupCreateRes getResponseType() {
        return new DCP_TaxGroupCreateRes();
    }
}
