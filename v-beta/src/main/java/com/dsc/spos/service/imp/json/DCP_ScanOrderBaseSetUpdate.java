...
import com.dsc.spos.json.cust.req.DCP_ScanOrderBaseSetUpdateReq;
import com.dsc.spos.json.cust.req.DCP_ScanOrderBaseSetUpdateReq.RangeList;
import com.dsc.spos.json.cust.res.DCP_ScanOrderBaseSetUpdateRes;
...
public class DCP_ScanOrderBaseSetUpdate extends SPosAdvanceService<DCP_ScanOrderBaseSetUpdateReq,DCP_ScanOrderBaseSetUpdateRes>
{

	@Override
	protected void processDUID(DCP_ScanOrderBaseSetUpdateReq req, DCP_ScanOrderBaseSetUpdateRes res) throws Exception 
	{
...
//		String area=null;
//
//		String reservedInvoiceinfo="0";
//		String orderInvoice="0";
//		String memberCarrier="0";
//
//		DCP_ScanOrderBaseSetUpdateReq.Invoice invoiceData=req.getRequest().getInvoice();
//		if(invoiceData!=null){
...
//				"RESTRICTLIKE","RESTRICTADVANCEORDER","RETAINTIME" ,"RECOMMENDEDDISHES" , 
//				"DESCRIPTION","SCANTYPE","CHECKTYPE","RESTRICTTABLE",
//				"RESTRICTSHOP", "RESTRICTCHANNEL"
//		};

		UptBean ub1 = null;
		ub1 = new UptBean("DCP_SCANORDER_BASESET");
		ub1.addUpdateValue("RULENAME", new DataValue(req.getRequest().getRuleName(), Types.VARCHAR));
//		ub1.addUpdateValue("PACKNO", new DataValue(req.getRequest().getPackNo(), Types.VARCHAR));
//		ub1.addUpdateValue("PACKNAME", new DataValue(req.getRequest().getPackName(), Types.VARCHAR));
		ub1.addUpdateValue("TABLESET", new DataValue(req.getRequest().getTableSet(), Types.VARCHAR));
		ub1.addUpdateValue("QRCODE", new DataValue(req.getRequest().getQrCode(), Types.VARCHAR));
		...
		ub1.addUpdateValue("LASTMODITIME", new DataValue(sNOW, Types.DATE));
		ub1.addUpdateValue("STATUS", new DataValue(status, Types.VARCHAR));
//		ub1.addUpdateValue("ISINVOICE", new DataValue(invoice, Types.VARCHAR));
//		ub1.addUpdateValue("AREA", new DataValue(area, Types.VARCHAR));
//		ub1.addUpdateValue("RESERVEDINVOICEINFO", new DataValue(reservedInvoiceinfo, Types.INTEGER));
//		ub1.addUpdateValue("ORDERINVOICE", new DataValue(orderInvoice, Types.INTEGER));
//		ub1.addUpdateValue("MEMBERCARRIER", new DataValue(memberCarrier, Types.INTEGER));
//		ub1.addUpdateValue("RESTRICTLIKE", new DataValue(restrictLike, Types.INTEGER));
		ub1.addUpdateValue("RESTRICTADVANCEORDER", new DataValue(restrictAdvanceOrder, Types.INTEGER));
		ub1.addUpdateValue("RETAINTIME", new DataValue(retainTime, Types.VARCHAR));
		
		ub1.addUpdateValue("RECOMMENDEDDISHES", new DataValue(recommendedDishes, Types.VARCHAR));
		ub1.addUpdateValue("DESCRIPTION", new DataValue(description, Types.VARCHAR));
		ub1.addUpdateValue("SCANTYPE", new DataValue(scanType, Types.VARCHAR));
		
		ub1.addUpdateValue("CHECKTYPE", new DataValue(checkType, Types.VARCHAR));
		ub1.addUpdateValue("RESTRICTTABLE", new DataValue(restrictTable, Types.VARCHAR));
		...
		ub1.addUpdateValue("REGISTER", new DataValue(restrictRegister, Types.VARCHAR));
		
		ub1.addUpdateValue("COUNTERPAY", new DataValue(counterPay, Types.VARCHAR));
		
		// 新增开关takeAway外带打包，coupon优惠券、integral积分 BY WANGZYC 20201123 
		ub1.addUpdateValue("TAKEAWAY", new DataValue(takeAway, Types.VARCHAR));// 打包带走 0.禁用 1.启用
		ub1.addUpdateValue("CONPON", new DataValue(coupon, Types.VARCHAR));// 优惠券 0.禁用 1.启用
		ub1.addUpdateValue("INTEGRAL", new DataValue(integral, Types.VARCHAR)); // 积分 0.禁用 1.启用 
		
		// 新增firstRegister点餐前会员注册 BY WANGZYC 20201130
		ub1.addUpdateValue("FIRSTREGISTER", new DataValue(firstRegister, Types.VARCHAR)); // 点餐前会员注册 0.禁用 1.启用
		
		// 新增orderMemo订单备注开关 BY WANGZYC 20201214
		ub1.addUpdateValue("ORDERMEMO", new DataValue(orderMemo, Types.VARCHAR)); // 订单备注 0.禁用 1.启用

		ub1.addUpdateValue("BEFORORDER", new DataValue(req.getRequest().getBeforOrder(), Types.VARCHAR)); // 预约点单 0.禁用 1.启用

        ub1.addUpdateValue("CHOOSABLETIME", new DataValue(req.getRequest().getChoosableTime(), Types.VARCHAR)); // 可选时间范围：近X天

        ub1.addUpdateValue("ISEVALUATEREMIND", new DataValue(req.getRequest().getIsEvaluateRemind(), Types.VARCHAR)); // 是否启用用餐评价提醒，0.禁用 1.启用
        ub1.addUpdateValue("REMINDTIME", new DataValue(req.getRequest().getRemindTime(), Types.VARCHAR)); // 下单X分钟后提醒

        // Add 2021-06-07 王欢 新增isAutoProm、isAutoRegister字段
        ub1.addUpdateValue("ISAUTOREGISTER", new DataValue(req.getRequest().getIsAutoRegister(), Types.VARCHAR)); // 是否自动注册会员 0.禁用 1.启用
        ub1.addUpdateValue("ISAUTOPROM", new DataValue(req.getRequest().getIsAutoProm(), Types.VARCHAR)); // 点单页促销提示 0.禁用 1.启用

        // Add 2021-09-27 应SA王欢需求 新增isAutoFold字段
        ub1.addUpdateValue("ISAUTOFOLD", new DataValue(req.getRequest().getIsAutoFold(), Types.VARCHAR)); // 售罄商品折叠 0.禁用 1.启用

        // Add 2022-02-15 应SA王欢需求 新增search字段
        ub1.addUpdateValue("SEARCH", new DataValue(req.getRequest().getSearch(), Types.VARCHAR)); // 点单页搜索 0.禁用 1.启用

        // Add 2022-02-28 应SA王欢需求 新增search字段
        ub1.addUpdateValue("RECHARGE", new DataValue(recharge, Types.VARCHAR)); // 结账提示充值 0.禁用 1.启用
		
		ub1.addUpdateValue("EVALUATION", new DataValue(evaluation, Types.VARCHAR)); //支付成功页评价 0.禁用 1.启用，默认禁用，空为禁用

        ub1.addUpdateValue("ISPAYCARD", new DataValue(isPayCard, Types.VARCHAR));
		
		ub1.addUpdateValue("ISGOODSDETAILDISPLAY", new DataValue(isGoodsDetailDisplay, Types.VARCHAR));
		

		// condition
		ub1.addCondition("EID", new DataValue(eId, Types.VARCHAR));
		ub1.addCondition("RULENO", new DataValue(ruleno, Types.VARCHAR));
		...
		{			
			...
			for (RangeList rangeMap : rangeList) {
				...
				if(!Check.Null(rangType) && rangType.equals("2")){
					shopList.add("'"+rangeMap.getId()+"'");
				}
				if(!Check.Null(rangType) && rangType.equals("3")){
			...
			if(!Check.Null(shopStr)){
				...
				sqlbuf.append("SELECT a.RULENO, a.RESTRICTCHANNEL, b.id AS shopid, c.id AS channelId " +
						...
						" WHERE a.eid = '"+eId+"' AND a.RESTRICTCHANNEL = '"+restrictChannel+"' AND b.ID IN ("+shopStr+")  and a.RESTRICTSHOP = '"+restrictShop+"' and a.RULENO <> '"+ruleno+"' ");
...
//				String channelSql = "SELECT * FROM Dcp_Scanorder_Baseset_Range A where A.EID='"+eId+"' and a.ruleNo != '"+ruleno+"' and A.rangeType = '3' and A.ID in ( "+channelStr+" ) " ;
...
//					}
					...
					strbuf.append(");</br>");
//					if(!Check.Null(exitRangeType) && exitRangeType.equals("3")&&restrictChannel.equals("1")){
...
//				}
			...
			DelBean db1 = new DelBean("DCP_SCANORDER_BASESET_RANGE");
...
//		{
			...
			if (restrictShop.equals("0")){
				String sql="select eid from dcp_scanorder_baseset t where t.eid='"+eId+"' and t.ruleNo != '"+ruleno+"' and t.status='100' and t.restrictShop= '0' ";	
...
//			if (restrictChannel.equals("0")){
//				String sql="select eid from dcp_scanorder_baseset t where t.eid='"+eId+"'  and t.ruleNo != '"+ruleno+"' and t.status='100' and t.restrictChannel= '0' ";
//				List<Map<String, Object>> getData=this.doQueryData(sql, null);
//				if (getData!=null && getData.size()>0)
//				{
//					res.setSuccess(false);
...
//			}
        ...
        DelBean db2 = new DelBean("DCP_SCANORDER_BASESET_PAYTYPE");
        db2.addCondition("EID", new DataValue(eId,Types.VARCHAR));
        db2.addCondition("RULENO", new DataValue(ruleno,Types.VARCHAR));
        this.addProcessData(new DataProcessBean(db2));

        List<DCP_ScanOrderBaseSetUpdateReq.PaySet> paySet = req.getRequest().getPaySet();
        if(!CollectionUtils.isEmpty(paySet)){
            String[] columns_PAYTYPE ={"EID","RULENO","SORTID","PAYTYPE","PAYNAME"};
            for (DCP_ScanOrderBaseSetUpdateReq.PaySet set : paySet) {
                DataValue[] insValue_PAYTYPE = new DataValue[]
                        {
                                new DataValue(eId, Types.VARCHAR),
                                new DataValue(ruleno, Types.VARCHAR),
                                new DataValue(set.getSortId(), Types.VARCHAR),
                                new DataValue(set.getPayType(), Types.VARCHAR),
                                new DataValue(set.getPayName(), Types.VARCHAR)
                        };
...
//		}
	...
	@Override
	protected List<InsBean> prepareInsertData(DCP_ScanOrderBaseSetUpdateReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	protected List<UptBean> prepareUpdateData(DCP_ScanOrderBaseSetUpdateReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	protected List<DelBean> prepareDeleteData(DCP_ScanOrderBaseSetUpdateReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	protected boolean isVerifyFail(DCP_ScanOrderBaseSetUpdateReq req) throws Exception 
	{
...
//		DCP_ScanOrderBaseSetUpdateReq.Invoice invoice=req.getRequest().getInvoice();
//		if(invoice!=null){
...
//        }
	...
	@Override
	protected TypeToken<DCP_ScanOrderBaseSetUpdateReq> getRequestType() {
		// TODO Auto-generated method stub
		return new TypeToken<DCP_ScanOrderBaseSetUpdateReq>(){};
	}

	@Override
	protected DCP_ScanOrderBaseSetUpdateRes getResponseType() {
		// TODO Auto-generated method stub
		return new DCP_ScanOrderBaseSetUpdateRes();
	}
