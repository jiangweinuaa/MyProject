...
import com.dsc.spos.json.cust.req.DCP_BankPayQueryReq;
import com.dsc.spos.json.cust.res.DCP_BankPayQueryRes;
...
public class DCP_BankPayQuery extends SPosBasicService<DCP_BankPayQueryReq, DCP_BankPayQueryRes> {
    @Override
    protected boolean isVerifyFail(DCP_BankPayQueryReq req) throws Exception {
        return false;
    }

    @Override
    protected TypeToken<DCP_BankPayQueryReq> getRequestType() {
        return new TypeToken<DCP_BankPayQueryReq>() {
        };
    }

    @Override
    protected DCP_BankPayQueryRes getResponseType() {
        return new DCP_BankPayQueryRes();
    }

    @Override
    protected DCP_BankPayQueryRes processJson(DCP_BankPayQueryReq req) throws Exception {
        DCP_BankPayQueryRes res = this.getResponseType();
        ...
        if (CollectionUtils.isNotEmpty(queryData)) {
            ...
            for (Map<String, Object> oneMaster : queryData) {
                DCP_BankPayQueryRes.Datas oneData = res.new Datas();
    ...
    @Override
    protected String getQuerySql(DCP_BankPayQueryReq req) throws Exception {
