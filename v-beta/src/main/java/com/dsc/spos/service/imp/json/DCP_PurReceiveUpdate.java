...
import com.dsc.spos.json.cust.req.DCP_PurReceiveUpdateReq;
import com.dsc.spos.json.cust.res.DCP_InterSettleDataGenerateRes;
import com.dsc.spos.json.cust.res.DCP_PurReceiveUpdateRes;
...
public class DCP_PurReceiveUpdate extends SPosAdvanceService<DCP_PurReceiveUpdateReq, DCP_PurReceiveUpdateRes> {
    @Override
    protected void processDUID(DCP_PurReceiveUpdateReq req, DCP_PurReceiveUpdateRes res) throws Exception {
        ...
        if (StringUtils.isNotEmpty(req.getRequest().getReceivingNo())) {
                ...
                req.getRequest().getDataList().forEach(x->{
                    if(Check.Null(x.getProcRate())&&Check.NotNull(x.getRItem())){
                        List<Map<String, Object>> filterRows = queryData.stream().filter(y -> x.getRItem().equals(y.get("ITEM").toString())).collect(Collectors.toList());
            ...
            String queryPurOrderNo = " SELECT a.*," +
                    "b.PURAMT,b.PLUNO,b.PURPRICE," +
                    "b.TAXCODE,b.TAXRATE,b.INCLTAX," +
                    "b.PURAMT,b.PURTEMPLATENO," +
                    "b.UNITRATIO,b.SUPPRICE,b.item," +
                    "b.RECEIVEPRICE,b.receiveamt,c.taxcode as taxcodey,c.taxrate as taxratey,c.item as itemy  " +
                    " FROM DCP_PURORDER a " +
...
//            }
        ...
        List<DCP_PurReceiveUpdateReq.DataList> giftFilterList = req.getRequest().getDataList().stream().filter(x -> Check.Null(x.getTaxCode()) ).collect(Collectors.toList());
        if(giftFilterList.size()>0){
            String ptemplateSql="select distinct a.PURTEMPLATENO,b.TAXCODE,b.pluno,c.taxrate,c.taxcaltype,c.incltax from DCP_PURCHASETEMPLATE a " +
            ...
            String querySupplier="select a.taxcode,c.taxrate,c.taxcaltype,c.incltax from DCP_BIZPARTNER a " +
        ...
        TableEntityDao entityDao = MySpringContext.getBean("entityDao");
            ...
            dcp_purReceive.add("LASTMODITIME", DataValues.newDate(lastModiTime));
        ...
        if(req.getRequest().getDataList().size()>0){
            List<String> collect = req.getRequest().getDataList().stream().filter(x -> Check.NotNull(x.getReceivingNo()) && Check.NotNull(x.getRItem())).map(x -> x.getRItem()).distinct().collect(Collectors.toList());
            if(collect.size()>0){

                List<String> errorMsg = new ArrayList<>();

                for (String item : collect){
                    List<DCP_PurReceiveUpdateReq.DataList> filterRows1 = req.getRequest().getDataList().stream().filter(x -> Check.NotNull(x.getRItem())&&x.getRItem().equals(item)).collect(Collectors.toList());
        ...
        List<DCP_PurReceiveUpdateReq.DataList> dataList = req.getRequest().getDataList();

            this.addProcessData(new DataProcessBean(DataBeans.getDelBean("dcp_purReceive_detail", condition)));

            String purDsql="select a.* from DCP_PURRECEIVE_DETAIL a where a.eid='"+req.geteId()+"' " +
        ...
        for (DCP_PurReceiveUpdateReq.DataList detail : dataList) {
                ...
                condition1.put("PLUNO", detail.getPluNo());
//                condition.put("FEATURENO",detail.getFeatureNo());
                List<Map<String, Object>> purData = MapDistinct.getWhereMap(getQueryPurOrder, condition1, true);
            ...
            if(Check.NotNull(detail.getTaxCode())){
                ...
                dcp_purReceive_detail.add("BILLNO", DataValues.newString(req.getRequest().getBillNo()));
...
//                dcp_purReceive_detail.add("PLUBARCODE",DataValues.newString(detail.getPluBarcode()));
                ...
                if (StringUtils.isNotEmpty(detail.getBatchNo())) {
                    List<Map<String, Object>> collect = purDList.stream().filter(x -> x.get("PLUNO").toString().equals(detail.getPluNo())
                            && x.get("FEATURENO").toString().equals(detail.getFeatureNo())
                            && x.get("PRODDATE").toString().equals(detail.getProdDate())
                            && x.get("EXPDATE").toString().equals(detail.getExpDate())).collect(Collectors.toList());
                    if(collect.size()<=0){
                        dcp_purReceive_detail.add("BATCHNO", DataValues.newString(PosPub.getBatchNo(dao, req.geteId(), req.getOrganizationNO(), "", detail.getPluNo(), detail.getFeatureNo(), detail.getProdDate(), detail.getExpDate(), req.getEmployeeNo(), req.getEmployeeName(), "",false)));
                    }else{
                        dcp_purReceive_detail.add("BATCHNO", DataValues.newString(detail.getBatchNo()));
                    }
...
//            this.addProcessData(new DataProcessBean(DataBeans.getUptBean("DCP_RECEIVING", conditionReceive, dcp_receiving)));
            ...
            if(!req.getRequest().getCorp().equals(req.getRequest().getBizCorp())) {
                ...
                request1.setBillNo(req.getRequest().getBillNo());
                request1.setSupplyOrgNo(req.getRequest().getBizOrgNo());
                request1.setDetail(new ArrayList<>());
                for (DCP_PurReceiveUpdateReq.DataList par : req.getRequest().getDataList()) {
                ...
                if (resserver.isSuccess() == false) {// 先不报错
    ...
    @Override
    protected List<InsBean> prepareInsertData(DCP_PurReceiveUpdateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected List<UptBean> prepareUpdateData(DCP_PurReceiveUpdateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected List<DelBean> prepareDeleteData(DCP_PurReceiveUpdateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected boolean isVerifyFail(DCP_PurReceiveUpdateReq req) throws Exception {
        return false;
    }

    @Override
    protected TypeToken<DCP_PurReceiveUpdateReq> getRequestType() {
        return new TypeToken<DCP_PurReceiveUpdateReq>() {
        };
    }

    @Override
    protected DCP_PurReceiveUpdateRes getResponseType() {
        return new DCP_PurReceiveUpdateRes();
    }
}
