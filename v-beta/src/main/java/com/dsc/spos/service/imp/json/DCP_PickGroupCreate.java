...
import com.dsc.spos.json.cust.req.DCP_PickGroupCreateReq;
import com.dsc.spos.json.cust.res.DCP_PickGroupCreateRes;
...
public class DCP_PickGroupCreate extends SPosAdvanceService<DCP_PickGroupCreateReq, DCP_PickGroupCreateRes> {
    @Override
    protected void processDUID(DCP_PickGroupCreateReq req, DCP_PickGroupCreateRes res) throws Exception {
        ...
        if (qData != null && !qData.isEmpty()) {
            throw new DispatchService.SPosCodeException(DispatchService.SPosCodeException.CODE_EXCEPTION_TYPE.E400,req.getOrganizationNO() + "已经存在分区编码"+req.getRequest().getPickGroupNo());
        }

        ColumnDataValue dcp_pickGroup = new ColumnDataValue();

        dcp_pickGroup.add("EID", DataValues.newString(req.geteId()));
        dcp_pickGroup.add("ORGANIZATIONNO", DataValues.newString(req.getOrganizationNO()));
        dcp_pickGroup.add("PICKGROUPNO", DataValues.newString(req.getRequest().getPickGroupNo()));
        ...
        dcp_pickGroup.add("CREATEOPID", DataValues.newString(req.getEmployeeNo()));
        dcp_pickGroup.add("CREATEDEPTID", DataValues.newString(req.getDepartmentNo()));
        dcp_pickGroup.add("CREATETIME", DataValues.newDate(DateFormatUtils.getNowDateTime()));
        ...
        this.addProcessData(new DataProcessBean(DataBeans.getInsBean("DCP_PICKGROUP", dcp_pickGroup)));

        for (DCP_PickGroupCreateReq.RangeList rangeList: req.getRequest().getRangeList()){
            ...
            dcp_pickGroupRange.add("CREATEOPID", DataValues.newString(req.getEmployeeNo()));
            dcp_pickGroupRange.add("CREATEDEPTID", DataValues.newString(req.getDepartmentNo()));
            dcp_pickGroupRange.add("CREATETIME", DataValues.newDate(DateFormatUtils.getNowDateTime()));
        ...
        if (null != req.getRequest().getObjectList()){
            for (DCP_PickGroupCreateReq.ObjectList objectList : req.getRequest().getObjectList()) {
                ...
                dcp_pickGroup_object.add("CREATEOPID", DataValues.newString(req.getEmployeeNo()));
                dcp_pickGroup_object.add("CREATEDEPTID", DataValues.newString(req.getDepartmentNo()));
                dcp_pickGroup_object.add("CREATETIME", DataValues.newDate(DateFormatUtils.getNowDateTime()));
    ...
    @Override
    protected List<InsBean> prepareInsertData(DCP_PickGroupCreateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected List<UptBean> prepareUpdateData(DCP_PickGroupCreateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected List<DelBean> prepareDeleteData(DCP_PickGroupCreateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected boolean isVerifyFail(DCP_PickGroupCreateReq req) throws Exception {
        return false;
    }

    @Override
    protected TypeToken<DCP_PickGroupCreateReq> getRequestType() {
        return new TypeToken<DCP_PickGroupCreateReq>() {
        };
    }

    @Override
    protected DCP_PickGroupCreateRes getResponseType() {
        return new DCP_PickGroupCreateRes();
    }
}
