...
import com.dsc.spos.json.cust.req.DCP_STakeTemplateUpdateReq;
import com.dsc.spos.json.cust.req.DCP_STakeTemplateUpdateReq.level1Elm;
import com.dsc.spos.json.cust.res.DCP_STakeTemplateUpdateRes;
 ...
 * @since  2016-09-20
 */
public class DCP_STakeTemplateUpdate extends SPosAdvanceService<DCP_STakeTemplateUpdateReq, DCP_STakeTemplateUpdateRes> {
	@Override
	protected boolean isVerifyFail(DCP_STakeTemplateUpdateReq req) throws Exception {
        ...
        List<DCP_STakeTemplateUpdateReq.OrgList> orgList = req.getRequest().getOrgList();
        if(CollUtil.isNotEmpty(orgList)) {
            for (DCP_STakeTemplateUpdateReq.OrgList org : orgList) {
	...
	@Override
	protected TypeToken<DCP_STakeTemplateUpdateReq> getRequestType() {
		return new TypeToken<DCP_STakeTemplateUpdateReq>(){};
	}

	@Override
	protected DCP_STakeTemplateUpdateRes getResponseType() {
		return new DCP_STakeTemplateUpdateRes();
	}

	@Override
	protected void processDUID(DCP_STakeTemplateUpdateReq req,DCP_STakeTemplateUpdateRes res) throws Exception {
		StringBuffer errMsg = new StringBuffer("");
		String templateNO = req.getRequest().getTemplateNo();
		...
		try {
			String sql= " select *  from DCP_PTEMPLATE  where EID= ? and DOC_TYPE='1' and PTEMPLATENO = ?  ";
			...
			if (getQData != null && !getQData.isEmpty()) {
				...
				ub1.addUpdateValue("IS_BTAKE", new DataValue(isBtake, Types.VARCHAR));
				...
				ub1.addUpdateValue("STOCKTAKECHECK", new DataValue(stockTakeCheck, Types.VARCHAR));
				...
				ub1.addUpdateValue("RANGEWAY", new DataValue(rangeWay, Types.VARCHAR));
				ub1.addUpdateValue("ISSHOWZSTOCK", new DataValue(isShowZStock, Types.VARCHAR));

				// condition
				ub1.addCondition("EID", new DataValue(eId, Types.VARCHAR));
				ub1.addCondition("PTEMPLATENO", new DataValue(templateNO, Types.VARCHAR));
				ub1.addCondition("DOC_TYPE", new DataValue("1", Types.VARCHAR));
				...
				if(shopType.equals("1")) {
					try {
						...
						db_shop.addCondition("DOC_TYPE", new DataValue("1", Types.VARCHAR));
	...
	@Override
	protected List<InsBean> prepareInsertData(DCP_STakeTemplateUpdateReq req) throws Exception {
		return null;
	}

	@Override
	protected List<UptBean> prepareUpdateData(DCP_STakeTemplateUpdateReq req) throws Exception {
		return null;
	}

	@Override
	protected List<DelBean> prepareDeleteData(DCP_STakeTemplateUpdateReq req) throws Exception {
		return null;
	}

	@Override
	protected String getQuerySql(DCP_STakeTemplateUpdateReq req) throws Exception {
		return null;
	}
