...
import com.dsc.spos.json.cust.req.DCP_SStockTemplateQueryReq;
import com.dsc.spos.json.cust.res.DCP_SStockTemplateQueryRes;
...
/**
 * 服务函数：DCP_SStockTemplateQuery
 * 服务说明：采购模板查询
 * @author Jinzma
 * @since  2017-03-09
 */
public class DCP_SStockTemplateQuery extends SPosBasicService<DCP_SStockTemplateQueryReq,DCP_SStockTemplateQueryRes> {
	
	@Override
	protected boolean isVerifyFail(DCP_SStockTemplateQueryReq req) throws Exception {
	...
	@Override
	protected TypeToken<DCP_SStockTemplateQueryReq> getRequestType() {
		return new TypeToken<DCP_SStockTemplateQueryReq>(){};
	}
	
	@Override
	protected DCP_SStockTemplateQueryRes getResponseType() {
		return new DCP_SStockTemplateQueryRes();
	}
	
	@Override
	protected DCP_SStockTemplateQueryRes processJson(DCP_SStockTemplateQueryReq req) throws Exception {
		DCP_SStockTemplateQueryRes res = this.getResponse();
		try {
			int totalRecords=0;
			int totalPages=0;
			//单头查询
			...
			if (getQData != null && !getQData.isEmpty()) {
				...
				for (Map<String, Object> oneData : getQData) {
					DCP_SStockTemplateQueryRes.level1Elm oneLv1 = res.new level1Elm();
	...
	@Override
	protected String getQuerySql(DCP_SStockTemplateQueryReq req) throws Exception {
		...
		sqlbuf.append(""
				...
				+ " where a.eid='"+eId+"' and a.doc_type='3'"
		);
		if (!Check.Null(keyTxt)) {
			sqlbuf.append(" and (a.ptemplateno like '%"+keyTxt+"%' or a.ptemplate_name like '%"+keyTxt+"%' "
					...
					+ " or a.supplier like '%"+keyTxt+"%' or c.supplier_name like '%"+keyTxt+"%') ");
		}
		...
		sqlbuf.append(" select * from ("
				...
				+ " a.ptemplateno,a.ptemplate_name,a.time_type,time_value,a.status,a.supplier,a.shoptype,"
				+ " c.supplier_name,c.abbr,a.create_date"
