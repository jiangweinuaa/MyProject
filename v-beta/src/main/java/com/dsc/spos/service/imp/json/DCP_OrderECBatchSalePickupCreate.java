...
//批量点货作业
public class DCP_OrderECBatchSalePickupCreate extends SPosAdvanceService<DCP_OrderECBatchSalePickupCreateReq,DCP_OrderECBatchSalePickupCreateRes> 
{

	Logger logger = LogManager.getLogger(DCP_OrderECBatchSalePickupCreate.class.getName());

	@Override
	protected void processDUID(DCP_OrderECBatchSalePickupCreateReq req, DCP_OrderECBatchSalePickupCreateRes res)
			throws Exception 
	{
		//		
		...
		StringBuffer sOrdernoMulti=new StringBuffer("");

		for (int i = 0; i < ecOrderno.length; i++) 
		{
			sOrdernoMulti.append(ecOrderno[i]+",");
		}
		...
		if(sOrdernoMulti.length()>0)
		{
            ...
            sOrdernoMulti.deleteCharAt(sOrdernoMulti.length()-1);
			//
			String[] arrPluno=new String[] {sOrdernoMulti.toString()};
...
//				reqUrl=PosPub.getDCP_URL(eId);
				...
				for(Map<String, Object> par: getQHeader)
				{
					...
					String getShop = par.get("ORDERSHOP").toString(); //取货门店
					String getShopName = par.get("ORDERSHOPNAME").toString();
					...
					//取单号因为本次事物还没提交所以还是上次的，需要处理一下
					String shipmentNo = this.getShipmentNO(req,shopId);
					//+1
					if (lstShipmentno.contains(shipmentNo)) 
					{					
						long i;
						shipmentNo = shipmentNo.substring(3, shipmentNo.length());
						i = Long.parseLong(shipmentNo) + 1;//+1
						shipmentNo = i + "";
						shipmentNo = "HYD" + shipmentNo; 						
					}
					
					lstShipmentno.add(shipmentNo);
					...
					//
					Map<String, Object> conditionD1 = new HashMap<String, Object>(); //查詢條件				
					conditionD1.put("ORDERNO", orderno);  
					//调用过滤函数			
					List<Map<String, Object>> getDetailDatas=MapDistinct.getWhereMap(getQDataDetail, conditionD1, true);
					
					//商品明细
					Map<String, Boolean> conditionD2 = new HashMap<String, Boolean>(); //查詢條件
					conditionD2.put("DETAIL_ITEM", true);	
					conditionD2.put("PLUNO", true);	
					//调用过滤函数
					List<Map<String, Object>> getD2_Detail=MapDistinct.getMap(getDetailDatas, conditionD2);
					
					//折扣明细	
					conditionD2.clear();
					conditionD2.put("AGIO_ITEM", true);	
					//调用过滤函数
					List<Map<String, Object>> getD3_Agio=MapDistinct.getMap(getDetailDatas, conditionD2);
					
					//付款明细
					conditionD2.clear();
					conditionD2.put("PAY_ITEM", true);	
					//调用过滤函数
					List<Map<String, Object>> getD4_Pay=MapDistinct.getMap(getDetailDatas, conditionD2);
					
					int bi=0;
					for (Map<String, Object> oneData2 : getD2_Detail) 
					{						
						//过滤属于此单头的明细
						...
						{							
							...
							//
							for (Map<String, Object> oneData3 : getD3_Agio) 
							{
								if(orderno.equals(oneData3.get("ORDERNO")) && oneData3.get("AGIO_ITEM").toString().equals("")==false && lstOrderAgioItem.contains(orderno+"_"+oneData3.get("AGIO_ITEM").toString())==false )
								{
									//订单折扣
									String agioItem=oneData3.get("AGIO_ITEM").toString();	
									lstOrderAgioItem.add(orderno+"_"+agioItem);

									String promName=oneData3.get("PROMNAME").toString();	
									String agioAmt=oneData3.get("AGIOAMT").toString();	
									String agioSellerDisc=oneData3.get("AGIO_SELLER_DISC").toString();	
									String agioPlatformDisc=oneData3.get("AGIO_PLATFORM_DISC").toString();
							...
							//
							for (Map<String, Object> oneData4 : getD4_Pay) 
							{
								if(orderno.equals(oneData4.get("ORDERNO")) && oneData4.get("PAY_ITEM").toString().equals("")==false && lstOrderPayItem.contains(orderno+"_"+oneData4.get("PAY_ITEM").toString())==false )
								{								
									//订单付款档
									String payItem=oneData4.get("PAY_ITEM").toString();	
									lstOrderPayItem.add(orderno+"_"+payItem);

									String payCode=oneData4.get("PAYCODE").toString();	
									String payCodeErp=oneData4.get("PAYCODEERP").toString();	
									String payName=oneData4.get("PAYNAME").toString();	
									String isOnlinePay=oneData4.get("ISONLINEPAY").toString();
									String pay=oneData4.get("PAY").toString();
									...
									pay_detail.put("canInvoice", oneData4.get("CANINVOICE").toString());
									pay_detail.put("invstartNo", oneData4.get("DINVSTARTNO").toString());									
									pay_detail.put("isOrderPay",oneData4.get("ISORDERPAY").toString() );
					...
					DataValue[] insValueOrderStatus_LOG = new DataValue[] 
							{ 
									...
									new DataValue("已點貨(批量)", Types.VARCHAR), // 状态名称
									...
									new DataValue("訂單狀態-->已點貨(批量)", Types.VARCHAR), //類型名稱+"-->"+狀態名稱
									new DataValue("100", Types.VARCHAR) 
							};
				...
				try 
				{
					...
					if(success)
					{
						logger.info("\r\nrequestId="+req.getRequestId()+",plantType="+req.getPlantType()+",version="+req.getVersion()+","+"" + "点货确定时 订转销处理, 转销售单成功，订单号="+sOrdernoMulti.toString());
					}
					else
					{
				...
				catch (Exception e) 
				{
					...
					logger.error("\r\nrequestId="+req.getRequestId()+",plantType="+req.getPlantType()+",version="+req.getVersion()+","+"服务***点货确定时，请求订转销处理前异常错误 OrderECSalePickupCreateDCP 返回参数 ：订单号=" +sOrdernoMulti.toString()+ "公司编码=" +eId+"\r\n"  + e.toString());
				}
	...
	@Override
	protected List<InsBean> prepareInsertData(DCP_OrderECBatchSalePickupCreateReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	protected List<UptBean> prepareUpdateData(DCP_OrderECBatchSalePickupCreateReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	protected List<DelBean> prepareDeleteData(DCP_OrderECBatchSalePickupCreateReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	protected boolean isVerifyFail(DCP_OrderECBatchSalePickupCreateReq req) throws Exception 
	{
	...
	@Override
	protected TypeToken<DCP_OrderECBatchSalePickupCreateReq> getRequestType() 
	{
		// TODO Auto-generated method stub
		return new TypeToken<DCP_OrderECBatchSalePickupCreateReq>() {};
	}

	@Override
	protected DCP_OrderECBatchSalePickupCreateRes getResponseType() 
	{
		// TODO Auto-generated method stub
		return new DCP_OrderECBatchSalePickupCreateRes();
	}
	 ...
	 * @throws Exception
	 */
	private String getShipmentNO(DCP_OrderECBatchSalePickupCreateReq req,String shopId) throws Exception  
	{
		/*
