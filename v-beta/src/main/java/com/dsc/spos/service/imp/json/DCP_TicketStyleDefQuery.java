...
import com.dsc.spos.json.cust.req.DCP_TicketStyleDefQueryReq;
import com.dsc.spos.json.cust.req.DCP_TicketStyleDetailQueryReq;
import com.dsc.spos.json.cust.res.DCP_TicketStyleDefQueryRes;
...
/**
 * 服务函数：DCP_TicketStyleDefQuery 
 * 服务说明：系统小票样式查询
 * @author wangzyc
 * @since 2020-12-3
 */
public class DCP_TicketStyleDefQuery extends SPosBasicService<DCP_TicketStyleDefQueryReq, DCP_TicketStyleDefQueryRes> {

	@Override
	protected boolean isVerifyFail(DCP_TicketStyleDefQueryReq req) throws Exception {
		// TODO 自动生成的方法存根
	...
	@Override
	protected TypeToken<DCP_TicketStyleDefQueryReq> getRequestType() {
		// TODO 自动生成的方法存根
		return new TypeToken<DCP_TicketStyleDefQueryReq>() {
		};
	}

	@Override
	protected DCP_TicketStyleDefQueryRes getResponseType() {
		// TODO 自动生成的方法存根
		return new DCP_TicketStyleDefQueryRes();
	}

	@Override
	protected DCP_TicketStyleDefQueryRes processJson(DCP_TicketStyleDefQueryReq req) throws Exception {
		// TODO 自动生成的方法存根
		String sql = null;
		DCP_TicketStyleDefQueryRes res = null;
		...
		try {
			...
			res.setDatas(new ArrayList<DCP_TicketStyleDefQueryRes.level1Elm>());
			...
			if (getQDataDetail != null && getQDataDetail.isEmpty() == false) {
				for (Map<String, Object> oneData : getQHeader) {
					// 取出第一层
					DCP_TicketStyleDefQueryRes.level1Elm oneLv1 = res.new level1Elm();
					...
					oneLv1.setCompoGroupList(new ArrayList<DCP_TicketStyleDefQueryRes.level2Elm>());
					for (Map<String, Object> oneData2 : getQHeader2) {
						...
						// 取出第二层
						DCP_TicketStyleDefQueryRes.level2Elm oneLv2 = res.new level2Elm();
						...
						oneLv2.setCompoList(new ArrayList<DCP_TicketStyleDefQueryRes.level3Elm>());
						...
						for (Map<String, Object> oneData3 : getQDataDetail) {
							...
							// 取出第三层
							DCP_TicketStyleDefQueryRes.level3Elm oneLv3 = res.new level3Elm();
	...
	@Override
	protected String getQuerySql(DCP_TicketStyleDefQueryReq req) throws Exception {
		// TODO 自动生成的方法存根
		...
		sqlbuf.append("SELECT a.STYLEID, a." + styleName + ", a.TICKETTYPE,"+ticketName+", c.groupId, c." + groupName + " , d.COMPOID,"
				+ " d." + compoName + ", d.CONTYPE, d.DEFVALUE, a.CONTENT , d.ALTERATEVALUE FROM DCP_TICKETSTYLE_DEF a "
				...
				+ "AND f.ENABLE = 'Y' WHERE b.RANGETYPE = '" + rangeType + "'");
		if (ticketType != null && ticketType.length() > 0) {
			sqlbuf.append("and a.TICKETTYPE LIKE '%%" + ticketType + "%%'");
		}
		if (keyTxt != null && keyTxt.length() > 0) {
			sqlbuf.append(
					"and (a.STYLEID LIKE '%%" + keyTxt + "%%' OR a." + styleName + " LIKE '%%" + keyTxt + "%%') ");
		}
	 ...
	 * @return
	 */
	private Map<String, Object> getNames(DCP_TicketStyleDefQueryReq req) {
