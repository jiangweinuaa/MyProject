...
import com.dsc.spos.json.cust.res.DCP_OrgCreateRes;
import com.dsc.spos.service.SPosAdvanceService;
import com.google.gson.reflect.TypeToken;

public class DCP_OrgCreate extends SPosAdvanceService<DCP_OrgCreateReq, DCP_OrgCreateRes> {

    @Override
    protected void processDUID(DCP_OrgCreateReq req, DCP_OrgCreateRes res) throws Exception {
        ...
        if (checkExisted(req.geteId(), req.getRequest().getOrganizationID())) {
            res.setSuccess(false);
            res.setServiceDescription(req.getRequest().getOrganizationID() + "已存在");
            return;
        }

        if(Check.Null(req.getRequest().getIsDept())){
            res.setSuccess(false);
            res.setServiceDescription("部门组织否不可为空");
            return;
        }

        if(Check.Null(req.getRequest().getIs_prod_org())){
            res.setSuccess(false);
            res.setServiceDescription("是否生产组织不可为空");
            return;
        }

        if(CollUtil.isNotEmpty(req.getRequest().getLicenseList())){
            for (DCP_OrgCreateReq.LicenseList licenseList:req.getRequest().getLicenseList()){
                if(Check.Null(licenseList.getImageType())){
                    res.setSuccess(false);
                    res.setServiceDescription("证照类型不可为空");
                    return;
                }

                if(Check.Null(licenseList.getLicenseImg())){
                    res.setSuccess(false);
                    res.setServiceDescription("证照图片文件名不可为空");
                    return;
                }

                if(Check.Null(licenseList.getLicenseImgUrl())){
                    res.setSuccess(false);
                    res.setServiceDescription("图片URL不可为空");
                    return;
                }
        ...
        if("Y".equals(req.getRequest().getIsDept())){
            String departSql="select * from dcp_department a where a.eid='"+req.geteId()+"' and a.DEPARTNO='"+req.getRequest().getOrganizationID()+"' ";
            List<Map<String, Object>> list = this.doQueryData(departSql, null);
            if(list.size()>0){
                res.setSuccess(false);
                res.setServiceDescription("部门"+req.getRequest().getOrganizationID()+"已存在");
                return;
            }
        }
        ...
        dcp_org.add("EID", DataValues.newString(req.geteId()));
        ...
        dcp_org.add("Org_Type", DataValues.newString(req.getRequest().getOrg_Type()));
        dcp_org.add("OrganizationNo", DataValues.newString(req.getRequest().getOrganizationID()));
...
//        dcp_org.add("SELFENDTIME",DataValues.newString(req.getRequest().getShopBeginTime()));
        ...
        dcp_org.add("CREATEDEPTID", DataValues.newString(req.getDepartmentNo()));
...
//        dcp_org.add("BILLCODE",DataValues.newDate(req.getRequest().getbillcode()));
        ...
        dcp_org.add("CREATEBY", DataValues.newString(req.getEmployeeNo()));
        dcp_org.add("MODIFYBY", DataValues.newString(req.getEmployeeNo()));
        dcp_org.add("CREATE_DTIME", DataValues.newDate(lastModifyTime));
        ...
        InsBean ins = DataBeans.getInsBean("DCP_ORG", dcp_org);
        ...
        if("Y".equals(req.getRequest().getIsDept())){
            ...
            if(CollUtil.isNotEmpty(req.getRequest().getDatasOrg_Lang())){
                ...
                for (DCP_OrgCreateReq.OrgLang orgLang:req.getRequest().getDatasOrg_Lang()){
        ...
        if (req.getRequest().getDataswareHouse() != null) {
            for (DCP_OrgCreateReq.Warehouse orgwh : req.getRequest().getDataswareHouse()) {
                ...
                if (orgwh.getDatasWare_Lang() != null) {
                    for (DCP_OrgCreateReq.WarehouseLang orgwhlang : orgwh.getDatasWare_Lang()) {
        ...
        if (req.getRequest().getShop_tag() != null && !req.getRequest().getShop_tag().isEmpty()) {
            for (DCP_OrgCreateReq.ShopTag shoptag : req.getRequest().getShop_tag()) {
        ...
        if (req.getRequest().getAccountList() != null && !req.getRequest().getAccountList().isEmpty()) {
            for (DCP_OrgCreateReq.Account shoptag : req.getRequest().getAccountList()) {
    ...
    private boolean checkExisted(String eId, String org) throws Exception {
        String querySql = " SELECT * FROM DCP_ORG WHERE EID='%s' AND OrganizationNo ='%s' ";
        querySql = String.format(querySql, eId, org);
    ...
    @Override
    protected List<InsBean> prepareInsertData(DCP_OrgCreateReq req) throws Exception {
        // TODO Auto-generated method stub
        return null;
    }

    @Override
    protected List<UptBean> prepareUpdateData(DCP_OrgCreateReq req) throws Exception {
        // TODO Auto-generated method stub
        return null;
    }

    @Override
    protected List<DelBean> prepareDeleteData(DCP_OrgCreateReq req) throws Exception {
        // TODO Auto-generated method stub
        return null;
    }

    @Override
    protected boolean isVerifyFail(DCP_OrgCreateReq req) throws Exception {
    ...
    @Override
    protected TypeToken<DCP_OrgCreateReq> getRequestType() {
        // TODO Auto-generated method stub
        return new TypeToken<DCP_OrgCreateReq>() {
        };
    }

    @Override
    protected DCP_OrgCreateRes getResponseType() {
        // TODO Auto-generated method stub
        return new DCP_OrgCreateRes();
    }
}
