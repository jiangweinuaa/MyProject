 ...
 *
 */
public class DCP_StockLock_Open extends SPosAdvanceService<DCP_StockLock_OpenReq, DCP_StockLock_OpenRes> {

	Logger logger = LogManager.getLogger(DCP_StockLock_Open.class.getName());
...
//			if(ecDatas != null && ecDatas.size() > 0 && ecDatas.isEmpty() == false){
...
//			}
//
//			logger.info("\r\nrequestId="+req.getRequestId()+",plantType="+req.getPlantType()+",version="+req.getVersion()+","+"********** DCP_StockLock 服务 查是否允许多机构出货："+ orderGoodsDeliver);
			...
			if(Check.Null(orgNo)){
				...
				String gaoDeKey = PosPub.getPARA_SMS(dao, eId, "", "AMAPWEBKEY");
...
//				}
				...
				if(orgDatas == null || orgDatas.size() < 1 || orgDatas.isEmpty()){
					throw new SPosCodeException(CODE_EXCEPTION_TYPE.E400, "无有效出货机构");
				}
...
//						else {  //锁定数量 <= 库存数量
							...
							logger.info("\r\nrequestId="+req.getRequestId()+",plantType="+req.getPlantType()+",version="+req.getVersion()+","+"********** DCP_StockLock 服务调用存储过程参数："+inputParameter.toString());
