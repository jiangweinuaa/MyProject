...
import com.dsc.spos.json.cust.req.DCP_PStockOutDetailQueryReq;
import com.dsc.spos.json.cust.res.DCP_PStockOutDetailQueryRes;
...
public class DCP_PStockOutDetailQuery extends SPosBasicService<DCP_PStockOutDetailQueryReq, DCP_PStockOutDetailQueryRes> {
    @Override
    protected boolean isVerifyFail(DCP_PStockOutDetailQueryReq req) throws Exception {
    ...
    @Override
    protected TypeToken<DCP_PStockOutDetailQueryReq> getRequestType() {
        return new TypeToken<DCP_PStockOutDetailQueryReq>(){};
    }

    @Override
    protected DCP_PStockOutDetailQueryRes getResponseType() {
        return new DCP_PStockOutDetailQueryRes();
    }

    @Override
    protected DCP_PStockOutDetailQueryRes processJson(DCP_PStockOutDetailQueryReq req) throws Exception {
        //2018-08-07添加docType;
        String docType = req.getRequest().getDocType(); //0-完工入库  1-组合单   2-拆解单   3-转换合并   4-合并转换

        //查询资料
        DCP_PStockOutDetailQueryRes res = this.getResponse();
        //单头单身查询
        ...
        if (getQData != null && !getQData.isEmpty()) {
            ...
            for (Map<String, Object> oneData : getQHeader) {
                DCP_PStockOutDetailQueryRes.level1Elm oneLv1 = res.new level1Elm();
                ...
                oneLv1.setMaterial_IsLocation(oneData.get("MATERIAL_ISLOCATION").toString());
                ...
                for (Map<String, Object> oneDataDetail : getDetailDatas) {
                    DCP_PStockOutDetailQueryRes.level2Elm oneLv2 = res.new level2Elm();
                    ...
                    oneLv2.setProdType(oneDataDetail.get("PRODTYPE").toString());
                    ...
                    for (Map<String, Object> oneMaterialData : getMaterialDatas) {
                        DCP_PStockOutDetailQueryRes.level3Elm oneLv3 = res.new level3Elm();
                        ...
                        String material_pqty = oneMaterialData.get("M_MATERIAL_PQTY").toString();
                        ...
                        oneLv3.setMaterial_pqty(material_pqty);
                    ...
                    oneLv2.setBatchList(new ArrayList<>());

                    BigDecimal totBatchQty=new BigDecimal("0");
                    List<Map<String, Object>> batchData = getBatchData.stream().filter(x -> x.get("OITEM").toString().equals(oneLv2.getItem())).collect(Collectors.toList());
                    for (Map<String, Object> oneBatchData : batchData){
                        DCP_PStockOutDetailQueryRes.BatchList batchList = res.new BatchList();
                        batchList.setItem(oneBatchData.get("ITEM").toString());
                        batchList.setOItem(oneBatchData.get("OITEM").toString());
                        batchList.setLocation(oneBatchData.get("LOCATION").toString());
                        batchList.setLocationName(oneBatchData.get("LOCATIONNAME").toString());
                        batchList.setBatchNo(oneBatchData.get("BATCHNO").toString());
                        batchList.setProdDate(oneBatchData.get("PRODDATE").toString());
                        batchList.setExpDate(oneBatchData.get("EXPDATE").toString());
                        batchList.setPUnit(oneBatchData.get("PUNIT").toString());
                        batchList.setPUnitName(oneBatchData.get("PUNITNAME").toString());
                        batchList.setPQty(oneBatchData.get("PQTY").toString());
                        batchList.setBaseUnit(oneBatchData.get("BASEUNIT").toString());
                        batchList.setBaseUnitName(oneBatchData.get("BASEUNITNAME").toString());
                        batchList.setBaseQty(oneBatchData.get("BASEQTY").toString());
                        totBatchQty.add(new BigDecimal(batchList.getBaseQty()));

                        oneLv2.getBatchList().add(batchList);
                ...
                for (DCP_PStockOutDetailQueryRes.level2Elm l2:oneLv1.getDatas()){
                    for (DCP_PStockOutDetailQueryRes.level3Elm l3:l2.getMaterial()){
                    ...
                    for (DCP_PStockOutDetailQueryRes.BatchList batchList:l2.getBatchList()){
                        List<Map<String, Object>> collectBatch = getBatchStockData.stream().filter(x -> x.get("PLUNO").toString().equals(l2.getPluNo()) &&
                                x.get("FEATURENO").toString().equals(l2.getFeatureNo()) &&
                                x.get("WAREHOUSE").toString().equals(l2.getWarehouse())&&
    ...
    @Override
    protected String getQuerySql(DCP_PStockOutDetailQueryReq req) throws Exception {
        ...
        sqlbuf.append(""
                ...
                + " b.location as m_material_location,b.expdate as m_material_expdate,l2.locationname as m_material_locationname,w2.islocation as material_islocation,b.costrate "
                + " from ("
                + " ");
        ...
        if( docType.equals("0")) {
            sqlbuf.append( " SELECT 1 as rw,0 as pqty_origin, 0 as pqty_refund,0 as SCRAP_QTY_ORIGIN, 0 as SCRAP_QTY_REFUND,"
                    ...
                    + " N'' as batch_no , N'' as prod_date,b.distriPrice,N'' as detail_memo,A.PROCESSPLANNO,A.TASK0NO,A.DTNO,dt1.dtname,dt1.begin_time,dt1.end_time,'' as employeeid,'' as departid,'' as CREATEDEPTID,'' as location,'' as expdate ,'' as prodType,'' as bomno,'' as versionnum  "
                    ...
                    + " where a.eid='"+ eId +"' and a.shopid='"+ shopId +"' and a.pdate = '"+ sDate +"'"
            );
        ...
        sqlbuf.append(" to_number(a.bDate) bdate,A.memo,A.status, A.oType  "
                ...
                + " dt1.dtname,dt1.begin_time,dt1.end_time,a.employeeid,a.departid,a.CREATEDEPTID,b.location,b.expdate,b.prodtype,b.bomno,b.versionnum  "
                + " FROM DCP_PSTOCKIN A  "
    ...
    private String getBatchSql(DCP_PStockOutDetailQueryReq req) throws Exception{
        StringBuffer sqlbuf = new StringBuffer();
        sqlbuf.append(" select a.*,b.locationname,u1.uname as punitname,u2.uname as baseunitname " +
                " from DCP_PSTOCKOUT_BATCH a " +
                " left join dcp_location b on a.eid=b.eid and a.location=b.location  and a.ORGANIZATIONNO=b.ORGANIZATIONNO  and a.WAREHOUSE=b.WAREHOUSE " +
