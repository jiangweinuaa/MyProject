...
import com.dsc.spos.json.cust.req.DCP_PFOrderOldCreateReq;
import com.dsc.spos.json.cust.req.DCP_PFOrderOldCreateReq.level1Elm;
import com.dsc.spos.json.cust.res.DCP_PFOrderOldCreateRes;
 ...
 *
 */
public class DCP_PFOrderOldCreate extends SPosAdvanceService<DCP_PFOrderOldCreateReq, DCP_PFOrderOldCreateRes> {
	
	Logger logger = LogManager.getLogger(this.getClass());
	
	@Override
	protected void processDUID(DCP_PFOrderOldCreateReq req, DCP_PFOrderOldCreateRes res) throws Exception {
...
//			
			String pfWeatherRatio = req.getRequest().getPfWeatherRatio();
			String pfHolidayRatio = req.getRequest().getPfHolidayRatio();
			String pfMatterRatio = req.getRequest().getPfMatterRatio();
//			
...
//			}
			...
			String[] columns1 = {
					...
					"PF_ID" , "PREDICTAMT", "BEGINDATE" , "ENDDATE" , "AVGSALEAMT" , "MODIFRATIO",
					"PFWEATHER" , "PFDAY", "PFHOLIDAY", "PFMATTER" 
			};
			...
			if(!checkGuid(req)){
				for (level1Elm par : jsonDatas) {
					...
					String[] columnsName = {
							...
							"BASEQTY", "PRICE", "AMT", "REQWQTY","KQTY", //13
...
//						//取库存单位、换算率
//							List<Map<String, Object>> getQData_Ratio = PosPub.getUnit_Ratio_Middle(dao,eId, pluNO, punit);//this.doQueryData(sql_Ratio, conditionValues_Ratio);
...
//							}
						...
						case 8:  
							...
							break;
						case 9:  
							keyVal = String.format("%.2f", baseQty);
//							keyVal = String.valueOf(wqty);     
						...
						case 21:  
							keyVal = par.getQty(); //实备数量默认等于实报数量
							break;	
						case 22:  
							String avgQty = "0";
...
//						}
				...
				insValue1 = new DataValue[]{
						...
						new DataValue(pfWeatherNo, Types.VARCHAR),
						new DataValue(pfDay, Types.VARCHAR),
						new DataValue(pfHolidayNo, Types.VARCHAR),
						new DataValue(pfMatterNo, Types.VARCHAR)
						
				};

				InsBean ib1 = new InsBean("DCP_PORDER_FORECAST", columns1);
				...
				logger.info("\r\nrequestId="+req.getRequestId()+",plantType="+req.getPlantType()+",version="+req.getVersion()+","+"bomSql:"+bomSql);
				...
				if(!bomDatas.isEmpty()){
					...
					for (int i = 0; i < materilDatas.size(); i++) {
						...
						String material_wUnit = materilDatas.get(i).get("MATERIAL_BASEUNIT").toString(); //原料库存单位
						String punit = materilDatas.get(i).get("MATERIAL_PUNIT").toString(); //原料要货单位
...
//								new DataValue(String.format("%."+pUnitUdLengthStr+"f", materialPqty - reqWQty), Types.VARCHAR),  // 实报数量 = 应报数量 - 实存
				...
				res.setServiceDescription("唯一标识符已存在，请勿重复提交！");
			}
		} catch (Exception e) {
	...
	@Override
	protected List<InsBean> prepareInsertData(DCP_PFOrderOldCreateReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	protected List<UptBean> prepareUpdateData(DCP_PFOrderOldCreateReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	protected List<DelBean> prepareDeleteData(DCP_PFOrderOldCreateReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	protected boolean isVerifyFail(DCP_PFOrderOldCreateReq req) throws Exception {
		// TODO Auto-generated method stub
	...
	@Override
	protected TypeToken<DCP_PFOrderOldCreateReq> getRequestType() {
		// TODO Auto-generated method stub
		return new TypeToken<DCP_PFOrderOldCreateReq>(){};
	}

	@Override
	protected DCP_PFOrderOldCreateRes getResponseType() {
		// TODO Auto-generated method stub
		return new DCP_PFOrderOldCreateRes();
	}
	...
	private boolean checkGuid(DCP_PFOrderOldCreateReq req) throws Exception {
		String sql = null;
		String guid = req.getRequest().getPfId();
		boolean existGuid;

		String[] conditionValues = { guid }; // 查询要货单号
	 ...
	 * @throws Exception 
	 */
	private String getPFNO(DCP_PFOrderOldCreateReq req) throws Exception{
	 ...
	 * @return
	 */
	protected String getBomDatas(DCP_PFOrderOldCreateReq req ,String pluNoStr){
		String eId = req.geteId();
		String shopId = req.getShopId();
