...
import com.dsc.spos.json.cust.req.DCP_StockOrderLockLogDetailReq;
import com.dsc.spos.json.cust.req.DCP_StockOrderLockLogDetailReq.LogChannelList;
import com.dsc.spos.json.cust.req.DCP_StockOrderLockLogDetailReq.OrganizationList;
import com.dsc.spos.json.cust.res.DCP_StockOrderLockLogDetailRes;
import com.dsc.spos.json.cust.res.DCP_StockOrderLockLogDetailRes.levelRes;
 ...
 *
 */
public class DCP_StockOrderLockLogDetail extends SPosBasicService<DCP_StockOrderLockLogDetailReq, DCP_StockOrderLockLogDetailRes> {

	@Override
	protected boolean isVerifyFail(DCP_StockOrderLockLogDetailReq req) throws Exception {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	protected TypeToken<DCP_StockOrderLockLogDetailReq> getRequestType() {
		// TODO Auto-generated method stub
		return new TypeToken<DCP_StockOrderLockLogDetailReq>(){};
	}

	@Override
	protected DCP_StockOrderLockLogDetailRes getResponseType() {
		// TODO Auto-generated method stub
		return new DCP_StockOrderLockLogDetailRes();
	}

	@Override
	protected DCP_StockOrderLockLogDetailRes processJson(DCP_StockOrderLockLogDetailReq req) throws Exception {
		// TODO Auto-generated method stub

		DCP_StockOrderLockLogDetailRes res = null;
		...
		try {
			...
			lvRes.setBillList(new ArrayList<DCP_StockOrderLockLogDetailRes.BillList>());
			...
			if(queryDatas.size() > 0 && !queryDatas.isEmpty()){
				...
				for (Map<String, Object> map : queryDatas) {
					DCP_StockOrderLockLogDetailRes.BillList lvPlu = res.new BillList();
					...
					String lockType = map.getOrDefault("LOCKTYPE","").toString();
					String lockTime = map.getOrDefault("LOCKTIME","").toString();

					String failFlag = map.getOrDefault("FAILFLAG","").toString();
					String failMsg = map.getOrDefault("FAILMSG","").toString();
...
//					lvPlu.setLockDate(lockDate);
					...
					lvPlu.setLockType(lockType);
					lvPlu.setFailFlag(failFlag);
					lvPlu.setFailMsg(failMsg);
	...
	@Override
	protected String getQuerySql(DCP_StockOrderLockLogDetailReq req) throws Exception {
		// TODO Auto-generated method stub
		String sql = null;
		...
		// channelList
		List<LogChannelList> channelDatas = req.getRequest().getChannelList();
		String channelStr = "";
		if(channelDatas != null && channelDatas.size() > 0 && !channelDatas.isEmpty()){
			String[] channelArr = new String[channelDatas.size()] ;
			// 新增时验证所有模板上有没有该参数
			int k = 0;
			for (LogChannelList channelList : channelDatas) {
				String channelId = "";

				if(!Check.Null(channelList.getChannelId())){
					channelId = channelList.getChannelId();
				}
				channelArr[k] = channelId ;
				k++;
		...
		sqlbuf.append(""
				+ " SELECT * from ( "
				+ " SELECT "
				+ " COUNT (*) OVER () AS NUM , dense_rank() over (order BY a.Id ) rn , "
		...
		if(!Check.Null(req.getRequest().getBillNo())){
			String billNo = req.getRequest().getPluNo();
