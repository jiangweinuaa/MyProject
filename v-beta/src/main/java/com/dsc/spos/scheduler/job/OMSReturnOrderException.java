 ...
 * OMS退货单转销售单失败的单号记录
 *
 * OMS_ReturnOrder_Exception 小key  eid_omsdocno
 *
 * 单据处理成功就从异常缓存中删除单据
 */
@PersistJobDataAfterExecution
@DisallowConcurrentExecution
public class OMSReturnOrderException extends InitJob
{
    ...
    Logger logger = LogManager.getLogger(OMSReturnOrderException.class.getName());

    static boolean bRun=false;//标记此服务是否正在执行中

    public  OMSReturnOrderException()
    {

    }
    public  OMSReturnOrderException(String eId,String shopId,String organizationNO, String billNo)
    {
        pEId=eId;
        pShop=shopId;
    ...
    public String doExe()  throws Exception
    {
        ...
        if (bRun && pEId.equals(""))
        {
            logger.info("\r\n*********获取分销OMS系统异常退货单OMSReturnOrderException正在执行中,本次调用取消:************\r\n");

            sReturnInfo="定时传输任务-获取分销OMS系统异常退货单OMSReturnOrderException正在执行中！";
            return sReturnInfo;
        }
        ...
        {
            sReturnInfo="定时传输任务-获取分销OMS系统异常退货单OMSReturnOrderException配置文件参数OMS_Url或OMS_Eid或OMS_ERP_Eid不能为空！";
            logger.error("\r\n*********"+sReturnInfo+"************\r\n");
            return sReturnInfo;
        }
        ...
        logger.info("\r\n*********获取分销OMS系统异常退货单OMSReturnOrderException定时调用Start:************\r\n");
        ...
        try
        {
            ...
            Map<String, String> values=redis.getALLHashMap("OMS_ReturnOrder_Exception");
            ...
            while(it.hasNext())
            {
                ...
                requestparam.put("ecaent",oms_eid);
                String ebadocno=jsonRedis.get("shipNo").toString();

                //重试2次
                String reTryCount="0";
                if (jsonRedis.has("reTryCount"))
                {
                    reTryCount=jsonRedis.get("reTryCount").toString();
                }

                int v_reTryCount=Integer.parseInt(reTryCount)+1;

                //缓存+1
                jsonRedis.put("reTryCount",v_reTryCount);

                //更新重试次数
                redis.setHashMap("OMS_ReturnOrder_Exception",oms_eid+"_"+ebadocno,jsonRedis.toString());
                if (v_reTryCount>2)
                {
                    continue;
                }

                requestparam.put("ecadocno",ebadocno);
                //requestparam.put("ecamdate_start",sdf_oms.format(sdf.parse(startTime)));//参数格式:2022/02/17 15:38:31
                //requestparam.put("ecamdate_end",sdf_oms.format(sdf.parse(endTime)));

                param.put("requestinterface","IOpenApiRequestService");
                param.put("requestmethod","getReturnOrders");
                param.put("requestmodel","I");
                param.put("requestparam",requestparam);
                head.put("param",param);

                logger.info("\r\n*********获取分销OMS系统异常退货单OMSReturnOrderException定时调用请求:"+head.toString()+"************\r\n");
                ...
                {
                    logger.error("\r\n*********获取分销OMS系统异常退货单OMSReturnOrderException 请求分销系统报错，返回为空************\r\n");
                }
                else
                {
                    ...
                    if (resStatus.equals("200"))
                    {
                        ...
                        if (Status.equals("0") || Status.equals(""))
                        {
                            ...
                            for (int i = 0; i < datas.length(); i++)
                            {
                                ...
                                try
                                {
                                    String a_eid=datas.getJSONObject(i).get("ecaent").toString();//企业编号
                                    a_billno=datas.getJSONObject(i).get("ecadocno").toString();//出货单号
                                    ...
                                    String a_createDate=datas.getJSONObject(i).get("ecaddate").toString();//单据日期 2021/08/12 11:01:02
                                    String a_platBillno=datas.getJSONObject(i).get("ecaord").toString();//平台单号
                                    String a_totAmt=datas.getJSONObject(i).get("ecatamt").toString();//含税金额
                                    //分销系统OMS的组织编码要与新零售编码一致，不做映射
                                    String a_shop=datas.getJSONObject(i).get("ecalorg").toString();//归属组织
                                    String a_orderShop=datas.getJSONObject(i).get("ecacorg").toString();//建立组织
                                    String a_shipShop=datas.getJSONObject(i).get("ecaiorg").toString();//入库组织
                                    String a_confirmNo=datas.getJSONObject(i).get("ecaaper").toString();//审核人
                                    String a_modifyNo=datas.getJSONObject(i).get("ecamper").toString();//修改人
                                    String a_modifyDate=datas.getJSONObject(i).get("ecamdate").toString();//修改日期 2021/08/12 11:01:02
                                    String a_memo=datas.getJSONObject(i).get("ecanote").toString();//备注
                                    String a_channel=datas.getJSONObject(i).get("ecachnl").toString();//渠道 00101
                                    String a_createDpt=datas.getJSONObject(i).get("ecacdept").toString();//建立部门 10000101
                                    String a_taxcode=datas.getJSONObject(i).get("ecatax").toString();//税种
                                    String a_taxRate=datas.getJSONObject(i).get("ecatrate").toString();//税率 13
                                    String a_getman=datas.getJSONObject(i).get("eca001").toString();//收货人
                                    String a_gettel=datas.getJSONObject(i).get("ecamtel").toString();//收货人电话
                                    String a_address=datas.getJSONObject(i).get("ecaaddr").toString();//详细地址
                                    String a_ebastatus=datas.getJSONObject(i).get("ecastus").toString();//单据状态 N.未审核 Y.已审核 S.已过账 X.已作废
                                    //未审核及作废的单据过滤掉
                                    ...
                                    sale.setgDate(sdf_Date.format(sdf_oms.parse(a_createDate)));
                                    sale.setgTime(sdf_Time.format(sdf_oms.parse(a_createDate)));
                                    ...
                                    JSONArray ebb_list=datas.getJSONObject(i).getJSONArray("ecb_list");
                                    for (int i1 = 0; i1 < ebb_list.length(); i1++)
                                    {
                                        String b_item=ebb_list.getJSONObject(i1).get("ecbseq").toString();//项次
                                        String b_pluno=ebb_list.getJSONObject(i1).get("ecbgoods").toString();//商品编号
                                ...
                                /*OMS不给就自己查,不过这样影响效能
                                String sql_goods="select PLU_NAME from dcp_goods_lang where eid='"+oms_eid+"' and pluno='"+b_pluno+"' and lang_type='"+langtype+"' ";
                                ...
                                */
                                        ...
                                        String b_featureNo=" ";//ebb_list.getJSONObject(i1).get("ecbgchr").toString();//特征
//                                        if (Check.Null(b_featureNo) || b_featureNo.equals("null"))
//                                        {
//                                            b_featureNo=" ";
//                                        }
                                        String b_qty=ebb_list.getJSONObject(i1).get("ecbqty").toString();//数量
                                        String b_unit=ebb_list.getJSONObject(i1).get("ecbtunit").toString();//销售单位
                                        String b_price=ebb_list.getJSONObject(i1).get("ecbprice").toString();//单价
                                        String b_amt=ebb_list.getJSONObject(i1).get("ecbtamt").toString();//含税金额
                                        String b_namt=ebb_list.getJSONObject(i1).get("ecbnamt").toString();//未税金额
                                        String b_warehouse=ebb_list.getJSONObject(i1).get("ecbwh").toString();//仓库
                                        ...
                                        inputParameter.put(25,"OMS退货加库存");                                //--异动描述
                                    ...
                                    //OMS电商退货单过账接口数据组装
                                    if (eaiList != null && eaiList.length()>0)
                                    {
                                        //OMS电商出货单过账接口
                                        ...
                                        {
                                            logger.error("\r\n*********获取分销OMS系统异常退货单OMSReturnOrderException "+a_billno+"过账操作请求分销系统报错，返回为空************\r\n");
                                            billToSale_ok=false;
                                            continue;
                                        }
                                        else
                                        {
                                            ...
                                            if (eai_resStatus.equals("200"))
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    logger.error("\r\n*********获取分销OMS系统异常退货单OMSReturnOrderException "+a_billno+"过账操作请求分销系统报错，返回内容:"+Message+"************\r\n");
                                                    billToSale_ok=false;
                                                    continue;
                                                }
                                            }
                                            ...
                                            {
                                                logger.error("\r\n*********获取分销OMS系统异常退货单OMSReturnOrderException "+a_billno+"过账操作请求分销系统报错，返回内容:"+resBody+"************\r\n");
                                                billToSale_ok=false;
                                                continue;
                                            }
                                        }
                                    ...
                                    InsBean ib_DCP_SALE = new InsBean("DCP_SALE", Columns_DCP_SALE);//分区字段
                                ...
                                catch (Exception e)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        logger.error("\r\n******获取分销OMS系统异常退货单OMSReturnOrderException报错单号:"+a_billno + e.getMessage()+"\r\n" + errors.toString() + "******\r\n");
                                ...
                                finally
                                {
                                    ...
                                    if (billToSale_ok)
                                    {
                                        redis.DeleteHkey("OMS_ReturnOrder_Exception",oms_eid+"_"+a_billno);
                                    }
                                }
                            }
                            //这里也成功了
                            if(datas.length()==0)
                            {
                                redis.DeleteHkey("OMS_ReturnOrder_Exception",oms_eid+"_"+ebadocno);
                            }
                        }
                        else
                        {
                            ...
                            logger.error("\r\n*********获取分销OMS系统异常退货单OMSReturnOrderException 请求分销系统报错，返回为"+Message+"************\r\n");
                        }
                    }
                    else
                    {
                        logger.error("\r\n*********获取分销OMS系统异常退货单OMSReturnOrderException 请求分销系统报错，返回为空"+resBody+"************\r\n");
                    }
                }
        ...
        catch (Exception e)
        {
            try
            {
                ...
                logger.error("\r\n******获取分销OMS系统异常退货单OMSReturnOrderException报错信息" + e.getMessage()+"\r\n" + errors.toString() + "******\r\n");
        ...
        {
            bRun=false;//
            logger.info("\r\n*********获取分销OMS系统异常退货单OMSReturnOrderException定时调用End:************\r\n");
        }
