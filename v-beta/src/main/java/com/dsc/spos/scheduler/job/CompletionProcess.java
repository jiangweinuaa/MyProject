...
public class CompletionProcess extends InitJob
{
	...
	public CompletionProcess()
	{

	}

	public CompletionProcess(String eId,String shopId,String organizationNO, String billNo)
	{				
		pEId=eId;
		pShop=shopId;
	...
	Logger logger = LogManager.getLogger(CompletionProcess.class.getName());
	...
	public String doExe() 
	{
		...
		if (bRun && pEId.equals(""))
		{		
			logger.trace("\r\n*********完工入库completion.process正在执行中,本次调用取消:************\r\n");

			sReturnInfo="定时传输任务-完工入库completion.process正在执行中！";
			return sReturnInfo;
		}

		bRun=true;//			

		logger.trace("\r\n*********完工入库completion.process定时调用Start:************\r\n");
		...
		try 
		{
			...
			logger.trace("\r\n******完工入库completion.process 执行SQL语句："+sql+"******\r\n");

			String[] conditionValues100_11 = {}; // 查詢條件
			List<Map<String, Object>> getQData100_11 = this.doQueryData(sql, conditionValues100_11);
			...
			if (getQData100_11 != null && getQData100_11.isEmpty() == false) 
			{
				for (Map<String, Object> oneData100 : getQData100_11) 
				{
					...
					String LOAD_DOCNO = oneData100.get("LOAD_DOCNO") == null ? "" : oneData100.get("LOAD_DOCNO").toString();
					...
					//2018-09-27 新增docType;
					String docType = oneData100.get("DOC_TYPE") == null ? "" : oneData100.get("DOC_TYPE").toString();
					...
					if (getQData101_11 != null && getQData101_11.isEmpty() == false)
					{
						...
						for (Map<String, Object> oneData101 : getQData101_11) 
						{
							...
							//2019-01-01 yyy 新增以下 报废数量 SCRAP_QTY  和  原因码 BSNO  
							String scrap_qty = oneData101.get("SCRAP_QTY") == null ? "0" : oneData101.get("SCRAP_QTY").toString();
							...
							if (getQData_adjustDetail != null && getQData_adjustDetail.isEmpty() == false)
							{
								for (Map<String, Object> oneData_adjustDeatil : getQData_adjustDetail) 
								{
									...
									String isBuckle = oneData_adjustDeatil.get("ISBUCKLE") == null ? "Y" : oneData_adjustDeatil.get("ISBUCKLE").toString();

									if(item.equals(material_Mitem))//完工入库的单身项次 ==库存调整单身的来源项次
									{ 
										...
										body_detail.put("material_is_buckle", isBuckle);
										body_detail.put("material_feature_no", material_featureNo);
							...
							body.put("base_qty", baseQty_b.toPlainString());
							body.put("invalidate_reason", BSNO); //这里传原因码信息
							body.put("invalidate_qty", scrap_qty);	//报废数量	  						
						...
						header.put("order_no", LOAD_DOCNO);
						header.put("order_shop", LOAD_DOCSHOP);

						//docType 0.完工入库    1.组合    2.拆解
						//operation_type 1.完工  2.取消   3.组合
						if(docType != null && docType.equals("0")){
							header.put("operation_type", "1"); // 1.完工
						}
						if(docType != null && docType.equals("1")){
							header.put("operation_type", "3"); // 3.组合
						}
						if(docType != null && docType.equals("3")){
							header.put("operation_type", "4"); // 4.转换合并
						}

                        header.put("account_date",accountDate);   //记账日期
						...
						logger.trace("\r\n******完工入库completion.process请求T100传入参数：  " + str + "\r\n");

						//执行请求操作，并拿到结果（同步阻塞）
						try 
						{
							String	resbody=HttpSend.Send(str, "completion.process", eId, shopId,organizationNO,PSTOCKINNO);
							logger.trace("\r\n" + "完工入库completion.process请求T100返回参数" + "\r\n单号="+ PSTOCKINNO + "\r\n" + resbody + "\r\n");
							...
							{				
								...
								InsertWSLOG.insert_WSLOG("completion.process",PSTOCKINNO,eId,organizationNO,"1",str,resbody,code,description);
							}
						...
						catch (Exception e) 
						{
							...
							InsertWSLOG.insert_WSLOG("completion.process",PSTOCKINNO,eId,shopId,"1",str,"","-1",e.getMessage());

							sReturnInfo="错误信息:" + e.getMessage();

							//System.out.println(e.toString());

							logger.error("\r\n服务***完工入库completion.process：门店=" +shopId+"组织编码=" + organizationNO + "公司编码=" +eId +" \n 单号="  + PSTOCKINNO  + e.toString());
						}
					...
					{
						logger.trace("\r\n*** 完工入库completion.process没单身数据****SQL="+sqlDetail+"********************************************************\r\n");
					}


					//
					if (getQData_adjustDetail != null)
					{
                        getQData_adjustDetail.clear();
                        getQData_adjustDetail=null;
					}

					//
					if (getQData101_11 != null)
					{
                        getQData101_11.clear();
                        getQData101_11=null;
					}
			...
			{
				logger.trace("\r\n***完工入库completion.process单头没数据****SQL="+sql+"********************************************************\r\n");
			}
		...
		{				
			try 
			{
				...
				logger.error("\r\n******完工入库completion.process报错信息" + e.getMessage()+"\r\n" + errors.toString() + "******\r\n");
	...
	//DCP_PSTOCKIN 获取单头信息
	protected String getQuerySql100_11() throws Exception 
	{
		String sql = null;
		StringBuffer sqlbuf = new StringBuffer("");
		sqlbuf.append(" select SHOPID,CONFIRM_DATE,CONFIRMBY,PSTOCKINNO,organizationno,EID,DOC_TYPE,MEMO,LOAD_DOCNO,LOAD_DOCSHOP,  " +
                " ACCOUNTBY,ACCOUNT_DATE from DCP_PSTOCKIN where  STATUS='2' and PROCESS_STATUS='N' "
				//2018-11-09 yyy 新增 docType in ('0','1')   0完工， 1 组合，2 拆解   3 转换合并
				+ " AND DOC_TYPE in ('0','1','3') and (load_doctype !='MES' or  load_doctype is null) ");
	...
	protected String getQuerySql101_11() throws Exception 
	{
		String sql = null;
		StringBuffer sqlbuf = new StringBuffer("");
		sqlbuf.append("select item,pluno,baseunit,baseqty,warehouse, bsno ,scrap_qty,batch_no,prod_date,featureno "
				+ " from DCP_PSTOCKIN_DETAIL where EID = ?  AND ORGANIZATIONNO = ? AND PSTOCKINNO = ? "
				+ " AND (SCRAP_QTY <>0 OR baseqty <> 0) ");
		sql = sqlbuf.toString();
		return sql;
	}
	...
	protected String getQuerySql_adjustDetail() throws Exception 
	{
		String sql = null;
		StringBuffer sqlbuf = new StringBuffer("");
