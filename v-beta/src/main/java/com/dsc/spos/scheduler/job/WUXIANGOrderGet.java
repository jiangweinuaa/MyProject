...
public class WUXIANGOrderGet extends InitJob
{
	//查询舞像的订单
	Logger logger = LogManager.getLogger(WUXIANGOrderGet.class.getName());

	static boolean bRun=false;//标记此服务是否正在执行中
	String jddjLogFileName = "WUXIANGOrderGet";
	public String doExe() throws Exception
	{
		String sReturnInfo = "";
		//官网物流推送单
		HelpTools.writelog_fileName("【同步任务WUXIANGOrderGet】同步START！",jddjLogFileName);
		//此服务是否正在执行中
		if (bRun)
		{		
			logger.info("\r\n*********同步任务WUXIANGOrderGet同步正在执行中,本次调用取消:************\r\n");
			HelpTools.writelog_fileName("【同步任务WUXIANGOrderGet】同步正在执行中,本次调用取消！",jddjLogFileName);
			return sReturnInfo;
		}
		bRun=true;//	
		//从缓存里面查询一下时间戳
		...
		{
			//这里开始查找舞像的定义
			String sql="select * from OC_ECOMMERCE where ECPLATFORMNO='7' ";
			...
			if(lisdate!=null&&!lisdate.isEmpty())
			{
				for (Map<String, Object> ECOMmap : lisdate) 
				{
					...
					String redis_key = "WMORDER" + "_" + "99" + "_" + "WUXIANG";
					...
					try
					{
						...
						while(true)
						{
							try
							{
								...
								js.put("start_timestamp", saledate);
								js.put("end_timestamp", enddate);
								//发送的内容
								HelpTools.writelog_fileName("【同步任务发送：" +"salesList "+ js.toString(),jddjLogFileName);
								String res= WUXIANGUtil.SendWuXiang("salesList", js.toString(), "http://preopenapi.bigaka.net/wx-api/newOpenApi",store_code);
								HelpTools.writelog_fileName("【同步任务返回：" +"salesList"+ res,jddjLogFileName);
								ParseJson pj=new ParseJson();

								OrderSalelist oslist=pj.jsonToBean(res,  new TypeToken<OrderSalelist>(){});

								if(oslist.getCode().equals("1"))
								{
									//调用成功，循环orders
									if(oslist.getResult().getOrders()==null)
									{
										break;
									}
									for (Orders map : oslist.getResult().getOrders()) 
									{
										//开始处理逻辑，需要循环取订单，然后插入数据库
										JSONObject jsdetail=new JSONObject();
										jsdetail.put("order_no", map.getOrder_no());

										//发送的内容
										HelpTools.writelog_fileName(store_code+"【同步任务发送：" +"salesDetail"+ jsdetail.toString(),jddjLogFileName);
										String resdetail= WUXIANGUtil.SendWuXiang("salesDetail", jsdetail.toString(), "http://preopenapi.bigaka.net/wx-api/newOpenApi",store_code);
										HelpTools.writelog_fileName(store_code+"【同步任务返回：" +"salesDetail"+ resdetail,jddjLogFileName);	
										Orderdetail osdetail=pj.jsonToBean(resdetail,  new TypeToken<Orderdetail>(){});
										if(osdetail.getCode().equals("1"))
										{
											//这里需要判断一下数据库里面有没有该单据，如果有可以直接continue掉
											String sqlorder="select * from OC_order where orderno='"+map.getOrder_no()+"'  "
													+ " and EID='"+eId+"' and LOAD_DOCTYPE='7'  ";
											List<Map<String, Object>> listsqlorder=this.doQueryData(sqlorder, null);
											if(listsqlorder!=null&&!listsqlorder.isEmpty())
											{
												HelpTools.writelog_fileName("【同步任务返回：" + map.getOrder_no()+"订单已存在！" ,jddjLogFileName);
												continue;
											}
											//转JSON 接口
											String res_json = GetWuXiangOrderResponse(osdetail,eId,PARTNER_ID);
											if(res_json==null||res_json.isEmpty())
											{
												HelpTools.writelog_fileName("【同步任务返回：" + map.getOrder_no()+"解析失败" ,jddjLogFileName);
												continue;
											}
										...
										{
											//调用失败
											HelpTools.writelog_fileName("【同步任务任返回：" + osdetail.getMessage(),jddjLogFileName);
											//							  			bRun=false;
											//											return sReturnInfo;
											issussce=false;
											break;
									...
									if(oslist.getResult().getOrders()==null||oslist.getResult().getOrders().size()<20)
									{	
										break;
									}
									i++;
								...
								{
									//调用失败
									HelpTools.writelog_fileName("【同步任务任返回：" + oslist.getMessage(),jddjLogFileName);
									//					  			bRun=false;
									//									return sReturnInfo;
									issussce=false;
									break;
	...
	public  String GetWuXiangOrderResponse(Orderdetail osdetail,String eId,String belfirm) throws Exception
	{
		...
		try 
		{
				...
				try
				{
					...
					if(curreginfomap.getStatus().equals("1"))
					{
						...
						String dsql="select * from (  select a.*,b.org_name ,F_CRM_GetDistance("+listjinwei[1]+","+listjinwei[0] +",a.LATITUDE,a.LONGITUDE) DISTANCE from DCP_ORG a "
								...
								+ " where a.eId='"+eId+"' and a.BELFIRM='"+belfirm+"' )"
			...
			//查询门店名称
			String sqlshop="select * from DCP_ORG_lang where organizationNO='"+wuxiangshop+"'  "
					+ " and eId='"+eId+"' and lang_type='zh_CN' ";
