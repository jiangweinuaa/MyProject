...
public class MES_SalesReturnConToErp extends InitJob
{
    ...
    Logger logger = LogManager.getLogger(MES_SalesReturnConToErp.class.getName());

    static boolean bRun=false;//标记此服务是否正在执行中

    public MES_SalesReturnConToErp()
    {

    }

    public MES_SalesReturnConToErp(String eId, String shopId, String organizationNO, String billNo)
    {
        pEId=eId;
        pShop=shopId;
    ...
    public String doExe()
    {
        ...
        if (bRun && pEId.equals(""))
        {
            logger.debug("\r\n*********MES确认后的销退单上传MES_SalesReturnConToErp正在执行中,本次调用取消:************\r\n");

            sReturnInfo = "定时传输任务-MES确认后的销退单上传MES_SalesReturnConToErp正在执行中！";
            return sReturnInfo;
        }

        bRun = true;//			

        logger.debug("\r\n*********MES确认后的销退单上传MES_SalesReturnConToErp定时调用Start:************\r\n");

        try
        {

            String sql = this.getQuerySql100_01();
            logger.info("\r\n******MES确认后的销退单上传MES_SalesReturnConToErp单头sql:" + sql);
            ...
            if (getQData100_01 != null && getQData100_01.isEmpty() == false)
            {
                ...
                for (Map<String, Object> oneData100: getQData100_01)
                {
                    ...
                    String salestockinno = oneData100.get("SALESTOCKINNO") == null ? "" : oneData100.get("SALESTOCKINNO").toString();
                    String erpNo = oneData100.get("ERPNO") == null ? "" : oneData100.get("ERPNO").toString();
                    ...
                    // 获取单身数据
                    sql = this.getQuerySql101_01(eId, organizationNo, salestockinno);
                    logger.info("\r\n******MES确认后的销退单上传MES_SalesReturnConToErp单身sql:" + sql);
                    ...
                    if (getQData101_01 != null	&& getQData101_01.isEmpty() == false)
                    {
                        ...
                        header.put("saleReturnNo", salestockinno);
                        header.put("erpNo", erpNo);
                        ...
                        header.put("mesSalesReturnCon_detail", request_detail);
                        request.put(header);

                        parameter.put("mesSalesReturnCon", request);
                        ...
                        try
                        {
                            logger.info("\r\n******MES确认后的销退单上传mesSalesReturnCon.create的数据:" + str);
                            resbody= HttpSend.Send(str, "mesSalesReturnCon.create", eId, organizationNo, organizationNo, salestockinno);
                            logger.info("\r\n******MES确认后的销退单上传mesSalesReturnCon.create erp返回的内容" + resbody);
                            ...
                            if (code.equals("0"))
                            {
                                ...
                                DataValue c3 = new DataValue(salestockinno, Types.VARCHAR);
                                conditions.put("SALESTOCKINNO", c3);

                                this.doUpdate("MES_SALESTOCKIN", values, conditions);
                                InsertWSLOG.delete_WSLOG(eId, organizationNo,"1", salestockinno);
                                //
                            ...
                            else
                            {
                                ...
                                //写数据库
                                InsertWSLOG.insert_WSLOG("mesSalesReturnCon.create",salestockinno,eId,organizationNo,"1",str,resbody,code,description) ;
                            }
                        }
                        catch (Exception e)
                        {
                            //
                            InsertWSLOG.insert_WSLOG("mesSalesReturnCon.create",salestockinno,eId,organizationNo,"1",str,resbody,"-1",e.getMessage());
                            sReturnInfo="错误信息:" + e.getMessage();
                            logger.error("\r\n******MES确认后的销退单上传mesSalesReturnCon.create：组织编码=" + organizationNo + ",公司编码=" +eId +",单号="  +salestockinno + "\r\n报错信息："+e.getMessage()+"******\r\n");
                        }

                    }
                    else
                    {
                        logger.debug("\r\n******MES确认后的销退单上传MES_SalesReturnConToErp,salestockinno="+salestockinno+"没有要上传的明细数据******\r\n");
                    }
            ...
            else
            {
                ...
                logger.debug("\r\n******MES确认后的销退单上传MES_SalesReturnConToErp,没有要上传的单头数据******\r\n");
            }
        ...
        catch (Exception e)
        {
            try
            {
                ...
                logger.error("\r\n******MES确认后的销退单上传MES_SalesReturnConToErp报错信息" + e.getMessage()+"\r\n" + errors.toString() + "******\r\n");
        ...
        {
            bRun=false;//
            logger.debug("\r\n*********MES确认后的销退单上传MES_SalesReturnConToErp定时调用End:************\r\n");
        }
        return sReturnInfo;
    }
    ...
    protected String getQuerySql100_01() throws Exception {
        ...
        sqlbuf.append("select * from ( "
                              + "select row_number() over(order by a.createtime) rn, a.*,b.ofno erpNo  " +
                              " from MES_SALESTOCKIN  a " +
                              " left join mes_salesreturn b on a.ofno = b.saleno and a.eid = b.eid and a.organizationno = b.organizationno " +
                              " where a.status=1 and b.status =1 and (a.process_status = 'N' or a.process_status is null) " +
                              " "
        );
    ...
    //MES_SALESRETURN_DETAIL
    protected String getQuerySql101_01(String eId, String organizationNo, String salestockinno) throws Exception {
        StringBuffer sqlbuf = new StringBuffer();
        sqlbuf.append("select a.*,b.oitem ERPOITEM from MES_SALESTOCKIN_DETAIL a  " +
                             " left join mes_salesreturn_detail b on a.eid = b.eid and a.organizationno = b.organizationno and a.ofno = b.saleno and a.oitem = b.item " +
                              " where a.eid='"+eId+"'  " +
                              " and a.organizationno='"+organizationNo+"'  " +
                              " and a.salestockinno='"+salestockinno+"' " +
                              " order by a.item  ");
        return sqlbuf.toString();
    }
