...
public class GuanYiOrderGet extends InitJob
{
	//查询管易的订单
	Logger logger = LogManager.getLogger(GuanYiOrderGet.class.getName());
	...
	public String doExe() throws Exception
	{
		...
		try
		{
			...
			if(lstProd!=null&&!lstProd.isEmpty())
			{
				...
				eId=lstProd.get(0).geteId().getValue();
			}
			//查询平台(微商城)是否启用调度
			String sql = "select A.*,B.org_name from OC_ECOMMERCE A left join DCP_ORG_lang B "
					+ " on A.eId=B.eId and A.SHOPID=B.Organizationno and B.lang_type='"+langtype+"' where A.eId='"+eId+"' and A.ECPLATFORMNO='13' and A.status='100'  ";
			...
			if(lisdate!=null&&!lisdate.isEmpty())
			{
				for (Map<String, Object> ECOMmap : lisdate) 
				{
					...
					String shopId=ECOMmap.get("SHOPID").toString();
					String WAREHOUSE=ECOMmap.get("WAREHOUSE").toString();

					RedisPosPub redis = new RedisPosPub();
					String redis_key = "WDMORDER" + "_" + "99" + "_" + "GUANYI";
					...
					try
					{
						...
						sql="select * from OC_mappingpayment where eId='' and load_doctype='13' and status='100'  ";
						List<Map<String, Object>> listpay=this.doQueryData(sql, null);
						...
						while(true)
						{
							try
							{
								...
								js.put("method","gy.erp.trade.deliverys.get");
								
								js.put("page_no", i);
								js.put("page_size", page_size);
								js.put("start_create", saledate);
								js.put("end_create", enddate);
								//js.put("warehouse_code", WAREHOUSE);
								//处理下sign
								...
								String err=jsres.getString("errorDesc");
								
								if(jsres.getBoolean("success"))
								{
									//调用成功，循环orders
									if(jsres.getJSONArray("deliverys").length()==0)
									{
										break;
									}
									
									for (int j = 0; j < jsres.getJSONArray("deliverys").length(); j++) 
									{
										JSONObject jsdetail=jsres.getJSONArray("deliverys").getJSONObject(j);

										String orderno=jsdetail.getString("code");
										String warehouse_code=jsdetail.getString("warehouse_code");
										
										//这里需要判断一下数据库里面有没有该单据，如果有可以直接continue掉
										String sqlorder="select * from OC_order where orderno='"+orderno+"'  and eId='"+eId+"' and LOAD_DOCTYPE='13'  ";
										...
										//转JSON 接口
										String res_json = GetWuXiangOrderResponse(jsdetail,eId,PARTNER_ID,listpay,lisdate);
									...
									if(jsres.getJSONArray("deliverys")==null||jsres.getJSONArray("deliverys").length()<20)
						  			{	
						  				break;
						  			}
						  			i++;
								...
								{
									//调用失败
									HelpTools.writelog_fileName("【同步任务任返回：" + err,jddjLogFileName);
									//					  			bRun=false;
									//									return sReturnInfo;
									issussce=false;
									break;
					   ...
					   while(true)
					   {
					  	 try
					  	 {
							...
							js.put("appkey", API_KEY);
							js.put("sessionkey",TOKEN);
							js.put("method","gy.erp.trade.return.get");
							...
							{
								//调用成功，循环orders
								if(jsres.getJSONArray("tradeReturns").length()==0)
								{
									break;
								}
								for (int j = 0; j < jsres.getJSONArray("tradeReturns").length(); j++) 
								{
									JSONObject jsdetail=jsres.getJSONArray("tradeReturns").getJSONObject(j);
									...
									String orderno=jsdetail.getString("code");
					  				...
					  				up.addCondition("LOAD_DOCTYPE", new DataValue("13", Types.VARCHAR));
					  				up.addCondition("ORDERNO", new DataValue(orderno, Types.VARCHAR));
					  				up.addCondition("EID", new DataValue(eId, Types.VARCHAR));
					  				
					  				up.addUpdateValue("STATUS", new DataValue("12", Types.VARCHAR));
					  				up.addUpdateValue("REFUNDSTATUS", new DataValue("6", Types.VARCHAR));
					  				up.addUpdateValue("DeliveryStutas", new DataValue("", Types.VARCHAR));
	...
	//舞像订单主动查询接口
	public  String GetWuXiangOrderResponse(JSONObject osdetail,String eId,String belfirm,List<Map<String, Object>> listpay,List<Map<String, Object>> lisdate) throws Exception
	{
		//解析收到的舞像请求 
		try 
		{
			String orderNo=osdetail.getString("code");
			...
			jsonobjresponse.put("customerNO", orderNo);						

			jsonobjresponse.put("status", "1");

			jsonobjresponse.put("loadDocType", "13");//1.饿了么 2.美团外卖 3.微商城	13管易云订单						
			...
			jsonobjresponse.put("isBook", "Y");//预定单

			jsonobjresponse.put("orderNO", orderNo);
			//String address = datasobj.get("address").toString();//
			String address=osdetail.getString("receiver_address");

			//String contactName = datasobj.get("contactName").toString();//联系人

			jsonobjresponse.put("contMan", osdetail.getString("receiver_name"));
			//String contactTelephone = datasobj.get("contactTelephone").toString();//
			jsonobjresponse.put("contTel", osdetail.getString("receiver_mobile"));

			jsonobjresponse.put("getMan", osdetail.getString("receiver_name"));
			//String contactTelephone = datasobj.get("contactTelephone").toString();//
			jsonobjresponse.put("getMantel", osdetail.getString("receiver_mobile"));
			
			jsonobjresponse.put("OUTSELID", osdetail.getString("vip_code"));
			...
			String status="0";
			jsonobjresponse.put("PSHOPREMIND", "1");
			
			try
			{
				//这里省 市 区 使用逗号隔开的
				String[] addlist=osdetail.getString("area_name").split("-");
				...
				if(curreginfomap.getStatus().equals("1"))
				{
					...
					//Map<String, Object> mshoptemp= matchshop(req, listjinwei[0], listjinwei[1]);
			...
			//日期为datetime类型
			String cdate= osdetail.get("create_date").toString(); //下单时间 2018-08-29 16:57:13
			...
			//这里处理下仓库订单，如果设置的是仓库是一样的那么就是走订单，不是这个仓库的单走发货单处理
			String warehouse_code=osdetail.getString("warehouse_code"); 
			String WAREHOUSE=lisdate.get(0).get("WAREHOUSE").toString(); 
			if(!warehouse_code.equals(WAREHOUSE))
			{
				jsonobjresponse.put("status", "2");
			    status="2";
			    //这里可以启用推荐门店
			    wuxiangshop=lisdate.get(0).get("SHOPID").toString();
			    shopwuxiangname=lisdate.get(0).get("ORG_NAME").toString();
				jsonobjresponse.put("shopId", wuxiangshop);//下单门店
				jsonobjresponse.put("organizationNO", wuxiangshop);
				jsonobjresponse.put("shopName", shopwuxiangname);
				jsonobjresponse.put("shippingShopNO", wuxiangshop);//配送门店=下单门店
				...
				jsonobjresponse.put("machShopNO", wuxiangshop);//生产门店，先默认 后面去指定
				jsonobjresponse.put("machShopName", shopwuxiangname);
				jsonobjresponse.put("isBook", "N");//预定单，作为后续定转销的标志
			}

			//日期是时间类型
			String shipdatetemp="";
			if(osdetail.get("plan_delivery_date")!=null&&!osdetail.get("plan_delivery_date").toString().isEmpty())
			{
				String wuxiangdate=osdetail.get("plan_delivery_date").toString();
				...
				jsonobjresponse.put("shipDate", shipdatetemp);//配送日期格式yyyyMMdd 20180808
			...
			String message =osdetail.get("buyer_memo").toString()=="null"?"":osdetail.getString("buyer_memo");//买家留言
			
			jsonobjresponse.put("memo", message);//单头备注
			//String totalAmount = datasobj.get("totalAmount").toString();//微商城 订单总应付金额

			jsonobjresponse.put("tot_oldAmt", osdetail.getDouble("amount")+osdetail.getDouble("post_fee"));//订单原价
			jsonobjresponse.put("tot_Amt", osdetail.getDouble("amount")+osdetail.getDouble("post_fee")-osdetail.getDouble("discount_fee"));//订单金额
			jsonobjresponse.put("incomeAmt", osdetail.getDouble("amount")+osdetail.getDouble("post_fee"));//商家实收金额

			//折扣金额
			jsonobjresponse.put("totDisc", osdetail.getDouble("discount_fee"));//商家实收金额
			jsonobjresponse.put("sellerDisc",osdetail.getDouble("discount_fee"));//商户优惠总额

			jsonobjresponse.put("manualNO",orderNo);//手工单号赋值订单号

			//String payedAmount = datasobj.get("payedAmount").toString();//微商城 已付金额
			jsonobjresponse.put("payAmt", osdetail.getDouble("payment") );//用户已支付金额

			//String deliverAmount = datasobj.get("deliverAmount").toString();//微商城 运费
			jsonobjresponse.put("shipFee", osdetail.getDouble("post_fee"));//配送费
			...
			boolean isgetpay=osdetail.getBoolean("cod");
			double payamt=osdetail.getDouble("payment");
			if(isgetpay)
			{
				if(payamt==0)
				{
			...
			String sno="";
			JSONArray array = osdetail.getJSONArray("details");
			...
			{				
				try 
				{
					...
					try 
					{
						price =goodsobj.getDouble("price");
					} 
					catch (Exception e) 
					{
					...
					try 
					{
						quantity_str = goodsobj.getDouble("qty")+"";
					...
					double goodsamt=goodsobj.getDouble("amount_after");
					double disc=goodsobj.getDouble("discount_fee");
					
					String pluno=goodsobj.getString("item_code");
					//查找一下PLUNO的商品品类
					if(sno.isEmpty())
					{
					...
					//处理下把规格放名称里面处理
					goodsobj.put("pluName", goodsobj.getString("item_name") );

					goodsobj.put("specName", goodsobj.getString("sku_name") );
			...
			jsonobjresponse.put("goods", arraytemp);
			//这里处理一下蛋糕还是西点订单
			if(sno.startsWith("2"))
			{
				//蛋糕订单,需要调度
			...
			if(payamt!=0)
			{
				...
				payobj.put("item", 1);
				payobj.put("payCode", listpay.get(0).get("PAYCODE"));
				payobj.put("payCodeerp", listpay.get(0).get("PAYCODEERP"));
				payobj.put("pay", payamt);
		...
		try
		{
			...
			else
			{
				if(req.getString("loadDocType").equals("4")||req.getString("loadDocType").equals("9"))
				{
					...
					//scur="select * from (  select a.*,b.org_name ,F_CRM_GetDistance("+lon+","+lat +",a.LONGITUDE,a.LATITUDE) DISTANCE from DCP_ORG a "
