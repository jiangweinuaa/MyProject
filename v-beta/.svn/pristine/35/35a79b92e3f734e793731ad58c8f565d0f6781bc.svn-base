...
public class MES_LStockOutToErp extends InitJob
{
    ...
    Logger logger = LogManager.getLogger(MES_LStockOutToErp.class.getName());

    static boolean bRun=false;//标记此服务是否正在执行中

    public MES_LStockOutToErp()
    {

    }

    public MES_LStockOutToErp(String eId, String shopId, String organizationNO, String billNo)
    {
        pEId=eId;
        pShop=shopId;
    ...
    public String doExe()
    {
        ...
        if (bRun && pEId.equals(""))
        {
            logger.debug("\r\n*********MES报损出库上传MES_LStockOutToErp正在执行中,本次调用取消:************\r\n");

            sReturnInfo = "定时传输任务-MES报损出库上传MES_LStockOutToErp正在执行中！";
            return sReturnInfo;
        }

        bRun = true;//			

        logger.debug("\r\n*********MES报损出库上传MES_LStockOutToErp定时调用Start:************\r\n");

        try
        {

            String sql = this.getQuerySql100_01();
            logger.info("\r\n******MES报损出库上传MES_LStockOutToErp单头sql:" + sql);
            ...
            if (getQData100_01 != null && getQData100_01.isEmpty() == false)
            {
                ...
                for (Map<String, Object> oneData100: getQData100_01)
                {
                    ...
                    String lStockOutNo = oneData100.get("LSTOCKOUTNO") == null ? "" : oneData100.get("LSTOCKOUTNO").toString();
                    ...
                    String tot_Qty = oneData100.get("TOT_PQTY") == null ? "" : oneData100.get("TOT_PQTY").toString();
                    ...
                    // 获取单身数据
                    sql = this.getQuerySql101_01(eId, organizationNo, lStockOutNo, ofno, warehouseNo);
                    logger.info("\r\n******MES报损出库上传MES_LStockOutToErp单身sql:" + sql);
                    ...
                    if (getQData101_01 != null	&& getQData101_01.isEmpty() == false)
                    {
                        ...
                        for (Map<String, Object> oneData101 : getQData101_01)
                        {
                            ...
                            String pluName = oneData101.get("PLUNAME") == null ? "" : oneData101.get("PLUNAME").toString();
                        ...
                        header.put("lStockOutNo", lStockOutNo);
                        ...
                        header.put("mesLStockOut_detail", request_detail);
                        request.put(header);

                        parameter.put("mesLStockOut", request);
                        ...
                        try
                        {
                            logger.info("\r\n******MES报损出库上传mesLStockOut.create的数据:" + str);
                            resbody= HttpSend.Send(str, "mesLStockOut.create", eId, organizationNo, organizationNo, lStockOutNo);
                            logger.info("\r\n******MES报损出库上传mesLStockOut.create erp返回内容" + resbody);
                            ...
                            if (code.equals("0"))
                            {
                                ...
                                DataValue c3 = new DataValue(lStockOutNo, Types.VARCHAR);
                                conditions.put("LSTOCKOUTNO", c3);
//                                DataValue c4 = new DataValue(1, Types.VARCHAR);
//                                conditions.put("STATUS", c4);
                                DataValue c5 = new DataValue(warehouseNo, Types.VARCHAR);
                                conditions.put("WAREHOUSE", c5);

                                this.doUpdate("MES_LSTOCKOUT", values, conditions);
                                InsertWSLOG.delete_WSLOG(eId, organizationNo,"1", lStockOutNo);
                                //
                            ...
                            else
                            {
                                ...
                                //写数据库
                                InsertWSLOG.insert_WSLOG("mesLStockOut.create",lStockOutNo,eId,organizationNo,"1",str,resbody,code,description) ;
                            }
                        }
                        catch (Exception e)
                        {
                            //
                            InsertWSLOG.insert_WSLOG("mesLStockOut.create",lStockOutNo,eId,organizationNo,"1",str,resbody,"-1",e.getMessage());
                            sReturnInfo="错误信息:" + e.getMessage();
                            logger.error("\r\n******MES报损出库上传mesLStockOut.create：组织编码=" + organizationNo + ",公司编码=" +eId +",报损单号="  +lStockOutNo + "\r\n报错信息："+e.getMessage()+"******\r\n");
                        }

                    }
                    else
                    {
                        logger.debug("\r\n******MES报损出库上传MES_LStockOutToErp,lStockOutNo="+lStockOutNo+"没有要上传的明细数据******\r\n");
                    }
            ...
            else
            {
                ...
                logger.debug("\r\n******MES报损出库上传MES_LStockOutToErp,没有要上传的单头数据******\r\n");
            }
    ...
    protected String getQuerySql101_01(String eId, String organizationNo, String lStockOutNo,String ofno,String warehouseNo) throws Exception {
        StringBuffer sqlbuf = new StringBuffer();
        sqlbuf.append("select a.* from MES_LSTOCKOUT_DETAIL a  " +
                              "where a.eid='"+eId+"'  " +
