...
public class sftcHttpClientUtil {
  ...
  public static String postRequest(String uri, String jsonParams) {
      ...
      try {
    	  HelpTools.writelog_fileName("【调用物流sftcHttpClientUtil】发送url:"+uri+",请求req:"+jsonParams,logFileName);
    	  ...
    	  try{
        	  HelpTools.writelog_fileName("【调用物流sftcHttpClientUtil】返回异常:"+e,logFileName);
          }catch (Exception ex) {
          }
      ...
      try{
    	  HelpTools.writelog_fileName("【调用物流sftcHttpClientUtil】返回res:"+output,logFileName);
      }catch (Exception e) {
      }
      return output;
  }
