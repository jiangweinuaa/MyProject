...
import com.dsc.spos.json.cust.res.DCP_ARDocCreateRes;
...
public class DCP_ARDocCreate extends SPosAdvanceService<DCP_ARDocCreateReq, DCP_ARDocCreateRes> {
    @Override
    protected void processDUID(DCP_ARDocCreateReq req, DCP_ARDocCreateRes res) throws Exception {

        String arNo = getNormalNO(req, "QTYS");
        ColumnDataValue dcp_arBill = new ColumnDataValue();
        dcp_arBill.add("EID", DataValues.newString(req.geteId()));
        dcp_arBill.add("CORP", DataValues.newString(req.getRequest().getCorp()));
...
//        dcp_arBill.add("REVSUBJECT", DataValues.newString(req.getRequest().getr()));
        ...
        int item = 1;
        for (DCP_ARDocCreateReq.ArBillList oneList : req.getRequest().getArBillList()) {
...
//            dcp_arBillDetail.add("CARDNO", DataValues.newString(oneList.getCateGory()));
        ...
        item = 1;
        for (DCP_ARDocCreateReq.ArPerdList onePerd:req.getRequest().getArPerdList()){
...
//            dcp_arPerd.add("LCYREVALADJNUM", DataValues.newString(onePerd.getlcyv()));
        ...
        for (DCP_ARDocCreateReq.ArBillSumList oneSum:req.getRequest().getArBillSumList()){
...
//            dcp_arBillDetailSum.add("TAXSUBJECTID", DataValues.newString(oneSum.getta()));
    ...
    @Override
    protected List<InsBean> prepareInsertData(DCP_ARDocCreateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected List<UptBean> prepareUpdateData(DCP_ARDocCreateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected List<DelBean> prepareDeleteData(DCP_ARDocCreateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected boolean isVerifyFail(DCP_ARDocCreateReq req) throws Exception {
        return false;
    }

    @Override
    protected TypeToken<DCP_ARDocCreateReq> getRequestType() {
        return new TypeToken<DCP_ARDocCreateReq>() {
        };
    }

    @Override
    protected DCP_ARDocCreateRes getResponseType() {
        return new DCP_ARDocCreateRes();
    }
}
