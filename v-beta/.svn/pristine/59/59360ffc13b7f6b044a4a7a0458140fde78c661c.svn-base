...
import com.dsc.spos.json.cust.req.DCP_CustomerPOrderCreateReq;
import com.dsc.spos.json.cust.req.DCP_InterSettleDataGenerateReq;
import com.dsc.spos.json.cust.res.DCP_CustomerPOrderCreateRes;
...
public class DCP_CustomerPOrderCreate extends SPosAdvanceService<DCP_CustomerPOrderCreateReq, DCP_CustomerPOrderCreateRes> {
    @Override
    protected boolean isVerifyFail(DCP_CustomerPOrderCreateReq req) throws Exception {
    ...
    @Override
    protected TypeToken<DCP_CustomerPOrderCreateReq> getRequestType() {
        return new TypeToken<DCP_CustomerPOrderCreateReq>() {
        };
    }

    @Override
    protected DCP_CustomerPOrderCreateRes getResponseType() {
        return new DCP_CustomerPOrderCreateRes();
    }

    @Override
    protected void processDUID(DCP_CustomerPOrderCreateReq req, DCP_CustomerPOrderCreateRes res) throws Exception {
        String lastModiTime = DateFormatUtils.getNowDateTime();
        String pOrderNo = getOrderNO(req, "DKYH");
        ...
        int i = 1;
        for (DCP_CustomerPOrderCreateReq.Datas oneData : req.getRequest().getDatas()) {
...
//                （一般纳税人）提示：商品xx未设定销项税别
            ...
            totCQty++;
            i++;
            oneData.setItem(String.valueOf(i));
            ...
            dcp_customerporder_detail.append("PORDERNO", DataValues.newString(pOrderNo));
            dcp_customerporder_detail.append("ITEM", DataValues.newString(i));
...
//                dcp_customerporder_detail.append("RETURNQTY", DataValues.newDecimal(oneData.getUnitRatio()));
        ...
        dcp_customerporder.append("EID", DataValues.newString(req.geteId()));
        dcp_customerporder.append("SHOPNO", DataValues.newString(req.getShopId()));
        dcp_customerporder.append("ORGANIZATIONNO", DataValues.newString(req.getOrganizationNO()));
        dcp_customerporder.append("PORDERNO", DataValues.newString(pOrderNo));
        ...
        dcp_customerporder.append("BDATE", DataValues.newString(DateFormatUtils.getNowPlainDate()));
        ...
        dcp_customerporder.append("CREATEOPID", DataValues.newString(req.getEmployeeNo()));
        dcp_customerporder.append("CREATEOPNAME", DataValues.newString(req.getEmployeeName()));
        dcp_customerporder.append("CREATETIME", DataValues.newDate(lastModiTime));
        ...
        dcp_customerporder.append("PAYER", DataValues.newString(req.getRequest().getPayer()));
        ...
        dcp_customerporder.append("CREATEDEPTID", DataValues.newString(req.getDepartmentNo()));

        this.addProcessData(new DataProcessBean(DataBeans.getInsBean("DCP_CUSTOMERPORDER", dcp_customerporder))); // 新增單頭
        ...
        if (!StringUtils.equals(corpData.get(req.getOrganizationNO()), corpData.get(req.getRequest().getDeliverOrgNo()))) {
            ...
            for (DCP_CustomerPOrderCreateReq.Datas par : req.getRequest().getDatas()) {
    ...
    @Override
    protected List<InsBean> prepareInsertData(DCP_CustomerPOrderCreateReq req) throws Exception {
        return null;
    }

    @Override
    protected List<UptBean> prepareUpdateData(DCP_CustomerPOrderCreateReq req) throws Exception {
        return null;
    }

    @Override
    protected List<DelBean> prepareDeleteData(DCP_CustomerPOrderCreateReq req) throws Exception {
        return null;
    }

    @Override
    protected String getQuerySql(DCP_CustomerPOrderCreateReq req) throws Exception {
        return null;
    }

//    private List<Map<String, Object>> getTaxCategory(String eId, String taxCode) throws Exception {
//        String getTaxSql = " SELECT TAXCALTYPE,TAXRATE*0.01 TAXRATE,INCLTAX FROM DCP_TAXCATEGORY WHERE EID='" + eId + "' and TAXCODE = '" + taxCode + "'";
//        List<Map<String, Object>> getTax = doQueryData(getTaxSql, null);
//        return getTax;
//    }


    private boolean checkGuid(DCP_CustomerPOrderCreateReq req) throws Exception {
        DCP_CustomerPOrderCreateReq.Request request = req.getRequest();
