...
public class WMMTService  extends SWaimaiBasicService {

	private Map<String, Object> req = null;

	String messageType ="";

	public WMMTService()//默认，防止更新兼容
	{
		this.messageType = "1";
	}

	public WMMTService(String messType)
	{
	...
	public String execute(String json) throws Exception {
		...
		{
			res_json = HelpTools.GetMTResponse(json);
		}
		else if (messageType.equals("2")) //取消类型消息
		{
			res_json = HelpTools.GetMTCancelResponse(json);		
		}
		else if (messageType.equals("3")) //退单类型消息
		{
			res_json = HelpTools.GetMTRefundResponse(json);						
		}
		else 
		{
