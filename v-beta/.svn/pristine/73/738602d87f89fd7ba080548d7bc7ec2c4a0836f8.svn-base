...
import com.dsc.spos.json.cust.req.DCP_PinPeiCreateReq;
import com.dsc.spos.json.cust.req.DCP_PinPeiCreateReq.level1Elm;
import com.dsc.spos.json.cust.res.DCP_PinPeiCreateRes;
 ...
 * @since  2020-07-13
 */
public class DCP_PinPeiCreate extends SPosAdvanceService<DCP_PinPeiCreateReq,DCP_PinPeiCreateRes>{

	@Override
	protected void processDUID(DCP_PinPeiCreateReq req, DCP_PinPeiCreateRes res) throws Exception {
		// TODO 自动生成的方法存根
		...
		try
		{
			String pinPeiId=req.getRequest().getPinPeiId();
			String sql = " select pinpeino from dcp_pinpei where pinpeiid='"+pinPeiId+"' ";
			...
			if (getQData == null || getQData.isEmpty())
			{
				...
				String pinPeiNo = getPinPeiNo(req);
				...
				////添加单头数据
				String[] columns = {
						"EID","SHOPID","PINPEINO","BDATE",
						"STOCKINWEAREHOUSE","STOCKOUTWEAREHOUSE",
						"TOT_PQTY","TOT_CQTY","TOT_AMT","TOT_DISTRIAMT",
						"MEMO","STATUS",
						"CREATEOPID","CREATEOPNAME","CREATETIME",
						"STOCKINBSNO","STOCKOUTBSNO","PINPEIID","CREATE_CHATUSERID",
				...
				DataValue[]	insValue = new DataValue[] {
						...
						new DataValue(pinPeiId, Types.VARCHAR),
						new DataValue(req.getChatUserId(), Types.VARCHAR),
						new DataValue(createBy, Types.VARCHAR),
						new DataValue(createByName, Types.VARCHAR),
						new DataValue(createTime, Types.DATE),
				};
				InsBean ib = new InsBean("DCP_PINPEI", columns);
				ib.addValues(insValue);
				this.addProcessData(new DataProcessBean(ib));
	...
	@Override
	protected List<InsBean> prepareInsertData(DCP_PinPeiCreateReq req) throws Exception {
		// TODO 自动生成的方法存根
		return null;
	}

	@Override
	protected List<UptBean> prepareUpdateData(DCP_PinPeiCreateReq req) throws Exception {
		// TODO 自动生成的方法存根
		return null;
	}

	@Override
	protected List<DelBean> prepareDeleteData(DCP_PinPeiCreateReq req) throws Exception {
		// TODO 自动生成的方法存根
		return null;
	}

	@Override
	protected boolean isVerifyFail(DCP_PinPeiCreateReq req) throws Exception {
		// TODO 自动生成的方法存根
		...
		if (Check.Null(pinPeiId))
		{
			errMsg.append("单据ID不能为空,");
			isFail = true;
		}
		if (Check.Null(bDate))
		{
	...
	@Override
	protected TypeToken<DCP_PinPeiCreateReq> getRequestType() {
		// TODO 自动生成的方法存根
		return new TypeToken<DCP_PinPeiCreateReq>(){};
	}

	@Override
	protected DCP_PinPeiCreateRes getResponseType() {
		// TODO 自动生成的方法存根
		return new DCP_PinPeiCreateRes();
	}

	private String getPinPeiNo(DCP_PinPeiCreateReq req) throws Exception {
		String eId=req.geteId();
		String shopId=req.getShopId();
		String bDate = PosPub.getAccountDate_SMS(dao, eId, shopId);
		String pinPeiNo= "PPZY" + bDate;
		String sql=" select max(pinpeino) as pinpeino from dcp_pinpei"
				+ " where eid='"+eId+"' and shopid='"+shopId+"' "
				+ " and pinpeino like '%%" + pinPeiNo + "%%'  ";
		List<Map<String, Object>> getQData = this.doQueryData(sql,null);
		pinPeiNo = getQData.get(0).get("PINPEINO").toString();
		if (Check.Null(pinPeiNo))
		{
			pinPeiNo = "PPZY" + bDate + "00001";
		}
		else
		{
			pinPeiNo = pinPeiNo.substring(4);
			long i = Long.parseLong(pinPeiNo) + 1;
			pinPeiNo = i + "";
			pinPeiNo = "PPZY" + pinPeiNo;
		}
