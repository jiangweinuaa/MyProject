...
import com.dsc.spos.scheduler.job.GuanYiGoodsUpdate.Goods;
import com.dsc.spos.scheduler.job.GuanYiGoodsUpdate.OrderSalelist;
import com.dsc.spos.scheduler.job.GuanYiGoodsUpdate.Orderdetail;
import com.dsc.spos.scheduler.job.GuanYiGoodsUpdate.Orders;
import com.dsc.spos.scheduler.job.GuanYiGoodsUpdate.Result;
import com.dsc.spos.scheduler.job.GuanYiGoodsUpdate.Resultdetail;
...
public class GuanYiSaleGet extends InitJob
{
	//管易订单自动转销售单处理
	Logger logger = LogManager.getLogger(GuanYiSaleGet.class.getName());

	static boolean bRun=false;//标记此服务是否正在执行中
	String jddjLogFileName = "GuanYiSaleGet";
	public String doExe() throws Exception
	{
		...
		try
		{
			//这里开始查找管易的需要中台做订转销的订单
			String sql="select * from OC_order where load_doctype='13' and status='2'  and isbook='N'  ";
			List<Map<String, Object>> lisdate=this.doQueryData(sql, null);
			DCP_OrderECBatchSale_OpenReq req=new DCP_OrderECBatchSale_OpenReq();
			
			List<String> listgyorder=new ArrayList();
			if(lisdate!=null&&!lisdate.isEmpty())
			{
				for (Map<String, Object> ECOMmap : lisdate) 
				{
					String ORDERNO=ECOMmap.get("ORDERNO").toString();
					String eId=ECOMmap.get("EID").toString();
					req.seteId(eId);
					req.setOpNO("admin");
					listgyorder.add(ORDERNO);
				}
			}
			req.setEcOrderNo(listgyorder.toArray(new String[listgyorder.size()]));
			req.setServiceId("DCP_OrderECBatchSale_Open");
			ParseJson js=new ParseJson();
			
			DispatchService ds = DispatchService.getInstance();
			String resbody = ds.callService(js.beanToJson(req), StaticInfo.dao);		
