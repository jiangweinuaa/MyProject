...
import com.dsc.spos.json.cust.req.DCP_ProcessPlanCreateReq;
import com.dsc.spos.json.cust.res.DCP_ProcessPlanCreateRes;
...
public class DCP_ProcessPlanCreate extends SPosAdvanceService<DCP_ProcessPlanCreateReq, DCP_ProcessPlanCreateRes>
{


    @Override
    protected void processDUID(DCP_ProcessPlanCreateReq req, DCP_ProcessPlanCreateRes res) throws Exception
    {
        ...
        //生产计划单
        String processplanno=PosPub.getBillNo(dao,req.getRequest().geteId(),req.getRequest().getOrganizationNo(),"SCJHD");
        ...
        for (DCP_ProcessPlanCreateReq.level2 level2 : req.getRequest().getDetailList())
        {
//            //转换基准单位
...
//            String unitRatio=map.get("unitRatio").toString();
        ...
        String[] columnsName_DCP_PROCESSPLAN = {
                "EID","ORGANIZATIONNO","SHOPID","PROCESSPLANNO","CREATE_DATE","CREATE_TIME","CREATEBY","PTEMPLATENO",
                "LOAD_DOCTYPE","LOAD_DOCNO","TOT_CQTY","TOT_PQTY","TOT_AMT","TOT_DISTRIAMT","PDATE_BEGIN","PDATE_END",
                "BDATE","MEMO","WAREHOUSE","MATERIALWAREHOUSE","WEEKDAY1","WEEKDAY2","WEEKDAY3","WEEKDAY4","WEEKDAY5",
                "WEEKDAY6","WEEKDAY7","AUTOCREATETASK","STATUS","CREATETIME","CREATEOPID","LASTMODIOPID","LASTMODITIME"
        };

        DataValue[] insValue_DCP_PROCESSPLAN = new DataValue[]
                {
                        new DataValue(req.getRequest().geteId(), Types.VARCHAR),
                        new DataValue(req.getRequest().getOrganizationNo(), Types.VARCHAR),
                        new DataValue(req.getRequest().getOrganizationNo(), Types.VARCHAR),
                        ...
                        new DataValue(req.getRequest().getpDateBegin(), Types.DATE),
                        new DataValue(req.getRequest().getpDateEnd(), Types.DATE),
                        new DataValue(sysDate, Types.VARCHAR),
                        ...
                        new DataValue(req.getRequest().getWeekDay1(), Types.VARCHAR),
                        new DataValue(req.getRequest().getWeekDay2(), Types.VARCHAR),
                        new DataValue(req.getRequest().getWeekDay3(), Types.VARCHAR),
                        new DataValue(req.getRequest().getWeekDay4(), Types.VARCHAR),
                        new DataValue(req.getRequest().getWeekDay5(), Types.VARCHAR),
                        new DataValue(req.getRequest().getWeekDay6(), Types.VARCHAR),
                        new DataValue(req.getRequest().getWeekDay7(), Types.VARCHAR),
                        new DataValue(req.getRequest().getAutoCreateTask(), Types.VARCHAR),
        ...
        InsBean ib_DCP_PROCESSPLAN = new InsBean("DCP_PROCESSPLAN", columnsName_DCP_PROCESSPLAN);
        ib_DCP_PROCESSPLAN.addValues(insValue_DCP_PROCESSPLAN);
        this.addProcessData(new DataProcessBean(ib_DCP_PROCESSPLAN));
    ...
    @Override
    protected List<InsBean> prepareInsertData(DCP_ProcessPlanCreateReq req) throws Exception
    {
        return null;
    }

    @Override
    protected List<UptBean> prepareUpdateData(DCP_ProcessPlanCreateReq req) throws Exception
    {
        return null;
    }

    @Override
    protected List<DelBean> prepareDeleteData(DCP_ProcessPlanCreateReq req) throws Exception
    {
        return null;
    }

    @Override
    protected boolean isVerifyFail(DCP_ProcessPlanCreateReq req) throws Exception
    {
    ...
    @Override
    protected TypeToken<DCP_ProcessPlanCreateReq> getRequestType()
    {
        return new TypeToken<DCP_ProcessPlanCreateReq>(){};
    }

    @Override
    protected DCP_ProcessPlanCreateRes getResponseType()
    {
        return new DCP_ProcessPlanCreateRes();
    }
