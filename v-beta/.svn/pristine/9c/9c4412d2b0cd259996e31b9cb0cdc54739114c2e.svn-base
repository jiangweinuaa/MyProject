...
import com.dsc.spos.json.cust.req.DCP_ScanOrderBaseSetQuery_OpenReq;
import com.dsc.spos.json.cust.res.DCP_ScanOrderBaseSetQuery_OpenRes;
...
public class DCP_ScanOrderBaseSetQuery_Open extends SPosBasicService<DCP_ScanOrderBaseSetQuery_OpenReq,DCP_ScanOrderBaseSetQuery_OpenRes>
{

	@Override
	protected boolean isVerifyFail(DCP_ScanOrderBaseSetQuery_OpenReq req) throws Exception {
		
		return false;
	}

	@Override
	protected TypeToken<DCP_ScanOrderBaseSetQuery_OpenReq> getRequestType() {
		
		return new TypeToken<DCP_ScanOrderBaseSetQuery_OpenReq>(){};
	}

	@Override
	protected DCP_ScanOrderBaseSetQuery_OpenRes getResponseType() {
		
		return new DCP_ScanOrderBaseSetQuery_OpenRes();
	}

	@Override
	protected DCP_ScanOrderBaseSetQuery_OpenRes processJson(DCP_ScanOrderBaseSetQuery_OpenReq req) throws Exception 
	{
		DCP_ScanOrderBaseSetQuery_OpenRes res = null;
		...
		StringBuffer sb = new StringBuffer("select * from (  "
				...
				+ " LEFT JOIN DCP_SCANORDER_BASESET_RANGE tc on t.eid = tc.eid and t.ruleNo = tc.ruleNo  "
				+ " where t.eid='" + eid + "' ");
		...
		if(!Check.Null(req.getApiUser().getChannelId()))
		{
			sb.append(" and ( " + " t.RESTRICTCHANNEL = '0' " + " or (t.RESTRICTCHANNEL = '1' and tc.id = '" + req.getApiUser().getChannelId() + "') "
					+ " or (t.RESTRICTCHANNEL = '2' and tc.id != '" + req.getApiUser().getChannelId() + "')" + ") ");
		}
		...
		res.setDatas(new ArrayList<DCP_ScanOrderBaseSetQuery_OpenRes.level1Elm>());
		for (Map<String, Object> oneData : getQHeader) {
			DCP_ScanOrderBaseSetQuery_OpenRes.level1Elm oneLv1 = res.new level1Elm();
...
//			oneLv1.setRestrictLike(oneData.get("RESTRICTLIKE").toString());
			...
			oneLv1.setRangeList(new ArrayList<DCP_ScanOrderBaseSetQuery_OpenRes.RangeList>());
			oneLv1.setPaySet(new ArrayList<DCP_ScanOrderBaseSetQuery_OpenRes.PaySet>());
			for (Map<String, Object> oneData2 : getRange) {
				DCP_ScanOrderBaseSetQuery_OpenRes.RangeList oneLv2 = res.new RangeList();
            ...
            for (Map<String, Object> set : getPaySet) {
                ...
                DCP_ScanOrderBaseSetQuery_OpenRes.PaySet paySet = res.new PaySet();
...
//
//			DCP_ScanOrderBaseSetQuery_OpenRes.Invoice invoice = new DCP_ScanOrderBaseSetQuery_OpenRes.Invoice();
...
//			oneLv1.setInvoice(invoice);
	...
	@Override
	protected String getQuerySql(DCP_ScanOrderBaseSetQuery_OpenReq req) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}
