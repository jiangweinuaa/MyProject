...
/**
 * 高铁接口：门店六月起交易流水上传
 * 这里专门设计为上传指定日期的数据， 目前高铁方接口不稳定，经常出现数据无法上传的情况。改动指定的日期时间即可
 * @author yuanyy
 *
 */
...
public class TradeChangeUploadAnyTime extends InitJob {
	//******兼容即时服务的,只查询指定的那张单据******
	String pEId="";
	String pShop="";	
	...
	public TradeChangeUploadAnyTime()
	{		
		
	}
	
	public TradeChangeUploadAnyTime(String eId,String shopId,String organizationNO, String billNo)
	{		
		pEId=eId;
		pShop=shopId;
    ...
    Logger logger = LogManager.getLogger(TradeChangeUploadAnyTime.class.getName());
	...
	public String doExe() 
	{
		...
		if (bRun && pEId.equals(""))
		{		
			logger.info("\r\n*********六月起交易流水上传TradeChangeUpload正在执行中,本次调用取消:************\r\n");
			
			sReturnInfo="定时传输任务-六月起交易流水上传TradeChangeUpload正在执行中！";
			return sReturnInfo;
		}

		bRun=true;//			
		
		logger.info("\r\n*********六月起交易流水上传TradeChangeUpload定时调用Start:************\r\n");
		...
		try 
		{
            ...
            + " WHERE  a.status='100' "
            + " and  ( a.item  in ('TradeKey' , 'TradeScale' , 'TradeShopId','TradeUrl' ,   "
            + "'TradeStationId' , 'TradeStationName','TradeShopName' )  "
            + " OR  b.item  in ('TradeKey' , 'TradeScale' , 'TradeShopId', 'TradeUrl' , "
            + "'TradeStationId' , 'TradeStationName','TradeShopName' )     )"
            + ") "
            + " ORDER BY SHOPID ";
			...
			if (getParamsDatas != null && getParamsDatas.isEmpty() == false) 
			{
				for (Map<String, Object> map : getParamsDatas) {
					...
					///////***********设置指定日期*********
					Date d1 = new SimpleDateFormat("yyyyMMdd").parse("20200408");//定义起始日期
					
					Date d2 = new SimpleDateFormat("yyyyMMdd").parse("20200424");//定义结束日期
					Calendar dd2 = Calendar.getInstance();//定义日期实例
					
					dd2.setTime(d1);//设置日期起始时间
					
					while(dd2.getTime().before(d2)){//判断是否到结束日期
				
						SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
				
						String str2 = sdf.format(dd2.getTime());
						System.out.println(str2);//输出日期结果
						dd2.add(Calendar.DAY_OF_MONTH, 1);//进行当前日期月份加1
						
						String beginTime = str2 + "000000";
						String endTime = str2 + "235959";
						

						//////**********开始调接口**********************
						String sql = this.getSaleDatas(shopId, TradeScale, eId, jobTime, beginTime , endTime );
						logger.info("\r\n******六月起交易流水上传TradeChangeUpload  SQL=" + sql + "******\r\n");
...
//									}
									...
									if( res.length() > 0 && res.indexOf("失败") != -1){
										...
										if(res.indexOf("违反唯一约束条件") != -1){
											...
											logger.error("\r\n******六月起交易流水上传TradeChangeUpload 失败 ，【"+orderNo+"】已上传单据"+"\r\n******\r\n");
										}
										else{
											logger.error("\r\n******六月起交易流水上传TradeChangeUpload \r\n 报错信息："+res+"******\r\n");
										}
									}
									else{
										logger.info("\r\n******六月起交易流水上传TradeChangeUpload 成功 "+"\r\n" + res + "******\r\n");
									}
...
//							} // while 循环结束
		...
		{					
			try 
			{
				...
				logger.error("\r\n******六月起交易流水上传TradeChangeUpload报错信息" + e.getMessage() +"\r\n" + errors.toString()+ "******\r\n");
		...
		{
			bRun=false;//
			logger.info("\r\n*********六月起交易流水上传TradeChangeUpload定时调用End:************\r\n");
		}			
	 ...
	 */
	protected String getSaleDatas(String shopId, String TradeScale, String eId, int jobTime, String beginTime, String endTime) throws ParseException{
		String sql = null;
