...
import com.dsc.spos.json.cust.req.DCP_InterSettSettingQueryReq;
import com.dsc.spos.json.cust.res.DCP_InterSettSettingQueryRes;
...
public class DCP_InterSettSettingQuery extends SPosBasicService<DCP_InterSettSettingQueryReq, DCP_InterSettSettingQueryRes> {

    @Override
    protected boolean isVerifyFail(DCP_InterSettSettingQueryReq req) throws Exception {
    ...
    @Override
    protected TypeToken<DCP_InterSettSettingQueryReq> getRequestType() {
        return new TypeToken<DCP_InterSettSettingQueryReq>(){};
    }

    @Override
    protected DCP_InterSettSettingQueryRes getResponseType() {
        return new DCP_InterSettSettingQueryRes();
    }

    @Override
    protected DCP_InterSettSettingQueryRes processJson(DCP_InterSettSettingQueryReq req) throws Exception {
        DCP_InterSettSettingQueryRes res = this.getResponse();
        int totalRecords=0;
        int totalPages=0;
        //单头查询
        ...
        if (getQData != null && !getQData.isEmpty()) {
            ...
            for (Map<String, Object> row : getQData){
                DCP_InterSettSettingQueryRes.levelElm level1Elm = res.new levelElm();
    ...
    @Override
    protected String getQuerySql(DCP_InterSettSettingQueryReq req) throws Exception {
        ...
        sqlbuf.append(""
                ...
                + " select a.PROCESSNO from DCP_INTERSETTSETTING a"
                + " where a.eid='"+eId+"' "
        );
        ...
        sqlbuf.append(" "
                + " select * from ("
                + " select count(*) over () num,row_number() over (order by a.PROCESSNO asc) as rn,"
                ...
                "a.cancelBy,a.cancel_Date,a.cancel_Time,c.org_name as supplyObjectName,d.org_name as demandObjectName,a.priceType   "
                + " from DCP_INTERSETTSETTING a"
