...
import com.dsc.spos.json.cust.req.DCP_N_OrderParaUpdateReq;
import com.dsc.spos.json.cust.res.DCP_N_OrderParaUpdateRes;
 ...
 * 服务说明：N_订单参数修改 新增一个参数isDinerIn 其余代码全部继承DCP_N_OrderParaUpdate
 * @author jinzma
 * @since  2024-05-23
 */
public class DCP_N_OrderParaUpdate extends SPosAdvanceService<DCP_N_OrderParaUpdateReq, DCP_N_OrderParaUpdateRes> {
    @Override
    protected void processDUID(DCP_N_OrderParaUpdateReq req, DCP_N_OrderParaUpdateRes res) throws Exception {
        ...
        try {
            ...
            DCP_N_OrderParaUpdateReq.level2Elm kdsSet = req.getRequest().getKdsSet();
...
//			ub.addUpdateValue("LASTMODITIME", new DataValue(lastmoditime, Types.DATE));
            ...
            String[] colNameOrderSet = {"EID", "ORGANIZATIONNO", "ISSELFPICK", "SELFBEGINTIME", "SELFENDTIME", "ISCITYDELIVERY",
                    ...
                    "ISQRCODE","DELIVERSHOP","ISSPARE","ISDINERIN"
            };
            ...
            DataValue[] insValueOrderSet =
                    {
                            ...
                            new DataValue(req.getRequest().getIsDinerIn(), Types.VARCHAR)     //是否堂食0否1是
                    };
...
//			this.addProcessData(new DataProcessBean(ub_orderSet));
            ...
            if (req.getRequest().getAutoTakeList() != null && !req.getRequest().getAutoTakeList().isEmpty())
            {
                ...
                for (DCP_N_OrderParaUpdateReq.level1Elm oneData : req.getRequest().getAutoTakeList())
                {
            ...
            if(radiateType.equals("1"))
            {
                if (req.getRequest().getRadiateShippingOrgList() != null && !req.getRequest().getRadiateShippingOrgList().isEmpty())
                {
                    ...
                    for (DCP_N_OrderParaUpdateReq.level1ShippingOrg oneData : req.getRequest().getRadiateShippingOrgList())
                    {
            ...
            List<DCP_N_OrderParaUpdateReq.level3Elm> stallList = kdsSet.getStallList();
            ...
            if(!CollectionUtils.isEmpty(stallList)){
                ...
                for (DCP_N_OrderParaUpdateReq.level3Elm level3Elm : stallList) {
                    DCP_N_OrderParaUpdateReq.func funcList = level3Elm.getFuncList();
                    DataValue[] insValue =
                            {
                                    new DataValue(req.geteId(), Types.VARCHAR),
                                    new DataValue(req.getRequest().getOrganizationNo(), Types.VARCHAR),
                    ...
                    List<DCP_N_OrderParaUpdateReq.subTag> subTagList = level3Elm.getSubTagList();
                    if(!CollectionUtils.isEmpty(subTagList)){

                        for (DCP_N_OrderParaUpdateReq.subTag subTag : subTagList) {
                ...
                List<DCP_N_OrderParaUpdateReq.level4Elm> imageList = kdsSet.getImageList();
                if(!CollectionUtils.isEmpty(imageList)){
                    for (DCP_N_OrderParaUpdateReq.level4Elm level4Elm : imageList) {
                        DataValue[] insOrgImg =
                                {
                                        new DataValue(req.geteId(), Types.VARCHAR),
                                        new DataValue(req.getRequest().getOrganizationNo(), Types.VARCHAR),
    ...
    @Override
    protected List<InsBean> prepareInsertData(DCP_N_OrderParaUpdateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected List<UptBean> prepareUpdateData(DCP_N_OrderParaUpdateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected List<DelBean> prepareDeleteData(DCP_N_OrderParaUpdateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected boolean isVerifyFail(DCP_N_OrderParaUpdateReq req) throws Exception {
        ...
        if (req.getRequest() == null) {
            ...
            List<DCP_N_OrderParaUpdateReq.level1Elm> autoTakeList = req.getRequest().getAutoTakeList();
            if (autoTakeList != null && !autoTakeList.isEmpty()) {
                for (DCP_N_OrderParaUpdateReq.level1Elm oneDate : autoTakeList) {
    ...
    @Override
    protected TypeToken<DCP_N_OrderParaUpdateReq> getRequestType() {
        return new TypeToken<DCP_N_OrderParaUpdateReq>(){};
    }

    @Override
    protected DCP_N_OrderParaUpdateRes getResponseType() {
        return new DCP_N_OrderParaUpdateRes();
    }
}
