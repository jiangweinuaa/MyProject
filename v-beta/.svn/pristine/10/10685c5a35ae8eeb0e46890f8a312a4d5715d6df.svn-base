...
public class MES_ProinAdjustToErp extends InitJob
{
    ...
    Logger logger = LogManager.getLogger(MES_ProinAdjustToErp.class.getName());

    static boolean bRun=false;//标记此服务是否正在执行中

    public MES_ProinAdjustToErp()
    {

    }

    public MES_ProinAdjustToErp(String eId, String shopId, String organizationNO, String billNo)
    {
        pEId=eId;
        pShop=shopId;
    ...
    public String doExe()
    {
        ...
        if (bRun && pEId.equals(""))
        {
            logger.debug("\r\n*********MES生产入库单上传MES_ProinAdjustToErp正在执行中,本次调用取消:************\r\n");

            sReturnInfo = "定时传输任务-MES生产入库单上传MES_ProinAdjustToErp正在执行中！";
            return sReturnInfo;
        }

        bRun = true;//			

        logger.debug("\r\n*********MES生产入库单上传MES_ProinAdjustToErp定时调用Start:************\r\n");

        try
        {

            String sql = this.getQuerySql100_01();
            logger.info("\r\n******MES生产入库单调整上传mesProinAdjust.create单头sql:" + sql);
            ...
            if (getQData100_01 != null && getQData100_01.isEmpty() == false)
            {
                ...
                for (Map<String, Object> oneData100: getQData100_01)
                {
                    ...
                    String adjustno = oneData100.get("ADJUSTNO") == null ? "" : oneData100.get("ADJUSTNO").toString();
                    String ofno = oneData100.get("OFNO") == null ? "" : oneData100.get("OFNO").toString();
                    String warehouseNo = oneData100.get("WAREHOUSE") == null ? "" : oneData100.get("WAREHOUSE").toString();
                    ...
                    String lastModifyTime = oneData100.get("LASTMODITIME") == null ? "" : oneData100.get("LASTMODITIME").toString();


                    // 获取单身数据
                    sql = this.getQuerySql101_01(eId, organizationNo, adjustno, ofno, warehouseNo);
                    logger.info("\r\n******MES生产入库单调整上传mesProinAdjust.create单身sql:" + sql);
                    ...
                    if (getQData101_01 != null	&& getQData101_01.isEmpty() == false)
                    {
                        ...
                        for (Map<String, Object> oneData101 : getQData101_01)
                        {
                            ...
                            String changeType = oneData101.get("CHANGETYPE") == null ? "" : oneData101.get("CHANGETYPE").toString();
                        ...
                        header.put("mesProinAdjust_detail", request_detail);
                        request.put(header);

                        parameter.put("mesProinAdjust", request);
                        ...
                        try
                        {
                            logger.info("\r\n******MES生产入库单调整上传mesProinAdjust.create的数据:" + str);
                            resbody= HttpSend.Send(str, "mesProinAdjust.create", eId, organizationNo, organizationNo, adjustno);
                            logger.info("\r\n******MES生产入库单调整上传mesProinAdjust.create erp返回的内容:" + resbody);
                            ...
                            if (code.equals("0"))
                            {
                                ...
                                DataValue c3 = new DataValue(adjustno, Types.VARCHAR);
                                conditions.put("ADJUSTNO", c3);
                                DataValue c5 = new DataValue(warehouseNo, Types.VARCHAR);
                                conditions.put("WAREHOUSE", c5);

                                this.doUpdate("MES_PROINADJUST", values, conditions);
                                InsertWSLOG.delete_WSLOG(eId, organizationNo,"1", adjustno);
                                //
                            ...
                            else
                            {
                                ...
                                //写数据库
                                InsertWSLOG.insert_WSLOG("mesProinAdjust.create",adjustno,eId,organizationNo,"1",str,resbody,code,description) ;
                            }
                        }
                        catch (Exception e)
                        {
                            //
                            InsertWSLOG.insert_WSLOG("mesProinAdjust.create",adjustno,eId,organizationNo,"1",str,resbody,"-1",e.getMessage());
                            sReturnInfo="错误信息:" + e.getMessage();
                            logger.error("\r\n******MES生产入库单调整上传mesProinAdjust.create：组织编码=" + organizationNo + ",公司编码=" +eId +",报损单号="  +adjustno + "\r\n报错信息："+e.getMessage()+"******\r\n");
                        }

                    }
                    else
                    {
                        logger.debug("\r\n******MES生产入库单调整上传MES_ProinAdjustToErp,adjustno="+adjustno+"没有要上传的明细数据******\r\n");
                    }
            ...
            else
            {
                ...
                logger.debug("\r\n******MES生产入库单调整上传MES_ProinAdjustToErp,没有要上传的单头数据******\r\n");
            }
        ...
        catch (Exception e)
        {
            try
            {
                ...
                logger.error("\r\n******MES报损出库上传MES_LStockOutToErp报错信息" + e.getMessage()+"\r\n" + errors.toString() + "******\r\n");
        ...
        {
            bRun=false;//
            logger.debug("\r\n*********MES报损出库上传MES_LStockOutToErp定时调用End:************\r\n");
        }
        return sReturnInfo;
    }
    ...
    //MES_LSTOCKOUT_DETAIL
    protected String getQuerySql101_01(String eId, String organizationNo, String adjustno,String ofno,String warehouseNo) throws Exception {
        StringBuffer sqlbuf = new StringBuffer();
        sqlbuf.append("select a.* from MES_PROINADJUST_DETAIL a  " +
                              "where a.eid='"+eId+"'  " +
