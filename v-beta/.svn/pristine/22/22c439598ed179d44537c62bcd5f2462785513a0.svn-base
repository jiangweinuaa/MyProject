...
public class MES_StockInToErp extends InitJob
{
    ...
    Logger logger = LogManager.getLogger(MES_StockInToErp.class.getName());

    static boolean bRun=false;//标记此服务是否正在执行中

    public  MES_StockInToErp()
    {

    }

    public  MES_StockInToErp(String eId,String shopId,String organizationNO, String billNo)
    {
        pEId=eId;
        pShop=shopId;
    ...
    public String doExe()
    {
        ...
        if (bRun && pEId.equals(""))
        {
            logger.debug("\r\n*********MES采购上传MES_StockInToErp mesStockIn.create正在执行中,本次调用取消:************\r\n");

            sReturnInfo = "定时传输任务-MES采购上传MES_StockInToErp mesStockIn.create正在执行中！";
            return sReturnInfo;
        }

        bRun = true;//			

        logger.debug("\r\n*********MES采购上传MES_StockInToErp mesStockIn.create定时调用Start:************\r\n");
        ...
        try
        {
            ...
            if (getQData100_01 != null && getQData100_01.isEmpty() == false)
            {
                ...
                for (Map<String, Object> oneData100: getQData100_01)
                {
                    ...
                    String stockinno = oneData100.get("STOCKINNO") == null ? "" : oneData100.get("STOCKINNO").toString();
                    String warehouse = oneData100.get("WAREHOUSENO") == null ? "" : oneData100.get("WAREHOUSENO").toString();
                    String ofno = "";
                    ...
                    String supplier = oneData100.get("SUPPLIER") == null ? "" : oneData100.get("SUPPLIER").toString();

                    // 获取单身数据
                    sql = this.getQuerySql101_01(organizationNO, eId, stockinno,warehouse);
                    ...
                    if (getQData101_01 != null	&& getQData101_01.isEmpty() == false)
                    {
                        ...
                        for (Map<String, Object> oneData101 : getQData101_01)
                        {
                            ...
                            //ERP的采购单号
                            ofno = oneData101.get("D_OFNO") == null ? "" : oneData101.get("D_OFNO").toString();
                        ...
                        header.put("stockInNo", stockinno);
                        header.put("ofNo", ofno);
                        header.put("warehouseNo", warehouse);
                        header.put("bDate", bdate);
                        header.put("opNo", createopid);
                        header.put("supplierNo", supplier);
                        header.put("version", "3.0");
                        header.put("messtockin_detail", request_detail);
                        request.put(header);

                        parameter.put("messtockin", request);
                        ...
                        try
                        {
                            resbody= HttpSend.Send(str, "mesStockIn.create", eId, organizationNO, organizationNO, stockinno);
                            ...
                            if (code.equals("0"))
                            {
                                ...
                                DataValue c3 = new DataValue(stockinno, Types.VARCHAR);
                                conditions.put("STOCKINNO", c3);

                                this.doUpdate("MES_STOCKIN", values, conditions);
                                InsertWSLOG.delete_WSLOG(eId, organizationNO,"1", stockinno);
                                //
                            ...
                            else
                            {
                                ...
                                //写数据库
                                InsertWSLOG.insert_WSLOG("mesStockIn.create",stockinno,eId,organizationNO,"1",str,resbody,code,description) ;
                            }
                        }
                        catch (Exception e)
                        {
                            //
                            InsertWSLOG.insert_WSLOG("mesStockIn.create",stockinno,eId,organizationNO,"1",str,resbody,"-1",e.getMessage());
                            sReturnInfo="错误信息:" + e.getMessage();
                            logger.error("\r\n******MES采购mesStockIn.create：组织编码=" + organizationNO + ",公司编码=" +eId +",单号="  +stockinno + "\r\n报错信息："+e.getMessage()+"******\r\n");
                        }

                    }
                    else 
                    {
                        logger.debug("\r\n******MES采购上传MES_StockInToErp mesStockIn.create,stockinno="+stockinno+"没有要上传的明细数据"+sql+"******\r\n");
                    }
                }
            }
            else
            {
                ...
                logger.debug("\r\n******MES采购上传MES_StockInToErp mesStockIn.create,没有要上传的单头数据"+sql+"******\r\n");
            }
        }
        catch (Exception e)
        {
            try
            {
                ...
                logger.error("\r\n******MES采购上传MES_StockInToErp mesStockIn.create报错信息" + e.getMessage()+"\r\n" + errors.toString() + "******\r\n");
        ...
        {
            bRun=false;//
            logger.debug("\r\n*********MES采购上传MES_StockInToErp mesStockIn.create定时调用End:************\r\n");
        }
        return sReturnInfo;
    }
    ...
    protected String getQuerySql100_01() throws Exception {
        ...
        sqlbuf.append("select * from ( "
                              + "select row_number() over(order by a.createtime) rn, a.*,b.OFNO B_DOCNO  " +
                              "from MES_STOCKIN a " +
                              "left join MES_STOCKMLRECEIPT_DETAIL b on a.eid=b.eid and a.organizationno=b.organizationno and a.OFNO=b.receiptno and b.item=1 " +
                              "where (a.process_status='N' or  a.process_status is null) " +
                              "and a.status=1 " +
                              " "
        );
    ...
    //MES_STOCKIN_DETAIL+MES_STOCKMLRECEIPT_DETAIL
    protected String getQuerySql101_01(String shopId, String eId, String stockinno,String warehouse) throws Exception {
        StringBuffer sqlbuf = new StringBuffer();
        sqlbuf.append("select a.*,b.OFNO,b.oitem,to_char(c.productdate,'yyyyMMdd') productdate,to_char(c.losedate,'yyyyMMdd') losedate,d.ofno D_OFNO " +
                              "from MES_STOCKIN_DETAIL a  " +
                              "left join MES_STOCKMLRECEIPT_DETAIL  b on a.eid=b.eid and a.organizationno=b.organizationno and a.OFNO=b.receiptno and a.oitem=b.item " +
                              "left join MES_BATCH c on a.eid=c.eid and a.pluno=c.pluno and nvl(a.featureno,' ')=nvl(c.featureno,' ') and a.batchno=c.batchno " +
                              "left join MES_RECEIVING d on a.eid=d.eid and a.organizationno=d.organizationno and a.oofno=d.receivingno " +
                              "where a.eid='"+eId+"'  " +
                              "and a.organizationno='"+shopId+"'  " +
                              "and a.warehouseno='"+warehouse+"' " +
