...
import com.dsc.spos.json.cust.req.DCP_CostAllocUpdateReq;
import com.dsc.spos.json.cust.res.DCP_CostAllocUpdateRes;
...
public class DCP_CostAllocUpdate extends SPosAdvanceService<DCP_CostAllocUpdateReq, DCP_CostAllocUpdateRes> {
    @Override
    protected void processDUID(DCP_CostAllocUpdateReq req, DCP_CostAllocUpdateRes res) throws Exception {
        ...
        List<DCP_CostAllocUpdateReq.AllocList> allocList = req.getRequest().getAllocList();
        ...
        condition.add("COSTCENTERNO", DataValues.newString(req.getRequest().getCostCenter()));

        this.addProcessData(new DataProcessBean(DataBeans.getDelBean("dcp_costalloc", condition)));

        for (DCP_CostAllocUpdateReq.AllocList alloc : allocList) {
    ...
    @Override
    protected List<InsBean> prepareInsertData(DCP_CostAllocUpdateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected List<UptBean> prepareUpdateData(DCP_CostAllocUpdateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected List<DelBean> prepareDeleteData(DCP_CostAllocUpdateReq req) throws Exception {
        return Collections.emptyList();
    }

    @Override
    protected boolean isVerifyFail(DCP_CostAllocUpdateReq req) throws Exception {
        return false;
    }

    @Override
    protected TypeToken<DCP_CostAllocUpdateReq> getRequestType() {
        return new TypeToken<DCP_CostAllocUpdateReq>() {
        };
    }

    @Override
    protected DCP_CostAllocUpdateRes getResponseType() {
        return new DCP_CostAllocUpdateRes();
    }
}
