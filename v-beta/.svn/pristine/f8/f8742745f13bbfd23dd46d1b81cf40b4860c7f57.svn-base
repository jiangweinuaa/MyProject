...
import com.dsc.spos.json.cust.req.DCP_PStockInUpdateReq;
import com.dsc.spos.json.cust.req.DCP_PStockInUpdateReq.level2Elm;
import com.dsc.spos.json.cust.req.DCP_PStockInUpdateReq.level1Elm;
import com.dsc.spos.json.cust.res.DCP_PStockInUpdateRes;
...
public class DCP_PStockInUpdate extends SPosAdvanceService<DCP_PStockInUpdateReq, DCP_PStockInUpdateRes> {
	
	@Override
	protected void processDUID(DCP_PStockInUpdateReq req, DCP_PStockInUpdateRes res) throws Exception {
			...
			if (checkGuid(req)) {
				...
				for (level1Elm par : datas) {
					...
					String[] columnsName = {
							...
							"ACCOUNT_DATE","FEATURENO","MEMO","LOCATION","EXPDATE","DISPTYPE","OOITEM","MINQTY","PRODTYPE","BOMNO","VERSIONNUM"
					};
                ...
                if(docType.equals("1")||docType.equals("0")){
                    ...
                    List<DCP_PStockInUpdateReq.StockInfo> stockInfos = getStockData.stream().map(x -> {
                        DCP_PStockInUpdateReq.StockInfo stockInfo = req.new StockInfo();
                    ...
                    for (DCP_PStockInUpdateReq.level1Elm par : datas){
                        List<DCP_PStockInUpdateReq.level2Elm> materials = par.getMaterial();
                        for (DCP_PStockInUpdateReq.level2Elm oneDataMaterial : materials) {
                            ...
                            List<DCP_PStockInUpdateReq.level3Elm> materialBatchList = oneDataMaterial.getMaterialBatchList();
                            if(CollUtil.isNotEmpty(materialBatchList)){
                                for (DCP_PStockInUpdateReq.level3Elm oneMb : materialBatchList){
                            ...
                            if(CollUtil.isNotEmpty(singlePluInfos)){
                                ...
                                if("Y".equals(isBatch)&&"Y".equals(isBatchNo)){
                                    List<DCP_PStockInUpdateReq.StockInfo> singlePluStock = stockInfos.stream().filter(x -> x.getPluNo().equals(material_pluNo) && x.getWarehouse().equals(material_warehouse)).collect(Collectors.toList());
                                    ...
                                    //int detailItem=0;
                                    for(DCP_PStockInUpdateReq.StockInfo oneStock : singlePluStock){
                                        //detailItem++;
                                        ...
                                        //stockInfos 扣减数量
                                        for (DCP_PStockInUpdateReq.StockInfo oldInfo :stockInfos){
                                    ...
                                    if(CollUtil.isNotEmpty(warehouseInfos)){
                                        ...
                                        if("N".equals(islocation)){
                                            ...
                                            //int detailItem=0;
                                            List<DCP_PStockInUpdateReq.StockInfo> singlePluStock = stockInfos.stream().filter(x -> x.getPluNo().equals(material_pluNo) && x.getWarehouse().equals(material_warehouse)).sorted(Comparator.comparing(x -> x.getLocation())).collect(Collectors.toList());
                                            for(DCP_PStockInUpdateReq.StockInfo oneStock : singlePluStock){
                                                //detailItem++;
                                                BigDecimal stockQty= new BigDecimal(oneStock.getQty());
                                                if(material_pqty.compareTo(BigDecimal.ZERO)<=0){
	...
	@Override
	protected List<InsBean> prepareInsertData(DCP_PStockInUpdateReq req) throws Exception {
		return null;
	}
	
	@Override
	protected List<UptBean> prepareUpdateData(DCP_PStockInUpdateReq req) throws Exception {
		return null;
	}
	
	@Override
	protected List<DelBean> prepareDeleteData(DCP_PStockInUpdateReq req) throws Exception {
		return null;
	}
	
	@Override
	protected boolean isVerifyFail(DCP_PStockInUpdateReq req) throws Exception {
	...
	@Override
	protected TypeToken<DCP_PStockInUpdateReq> getRequestType() {
		return new TypeToken<DCP_PStockInUpdateReq>(){};
	}
	
	@Override
	protected DCP_PStockInUpdateRes getResponseType() {
		return new DCP_PStockInUpdateRes();
	}
	
	private boolean checkGuid(DCP_PStockInUpdateReq req) throws Exception {
		String eId= req.geteId();
		String shopId=req.getShopId();
		String pStockInNO = req.getRequest().getpStockInNo();
