<template>
  <div class="stock-analysis">
    ...
    <div class="page-header">
      ...
      <span class="page-title">商品库存分析</span>
    </div>
    ...
    <el-row :gutter="10" class="data-cards">
      ...
      <el-col :xs="24" :sm="12" :md="6">
        <el-card shadow="hover" class="data-card">
          <div class="card-header">
            <el-icon class="card-icon" style="color: #F56C6C"><List /></el-icon>
            <span>商品种类</span>
          </div>
          <div class="card-value">{{ formatNumber(summaryData.goodsCount) }}</div>
          <div class="card-footer">
            <span class="trend-label">库存商品数</span>
          </div>
        </el-card>
      </el-col>
    </el-row>
    ...
    <el-card class="table-card" shadow="hover">
      ...
      <el-table :data="stockList" style="width: 100%" v-loading="loading" :default-sort="{prop: 'STOCK_AMT', order: 'descending'}">
        ...
        <el-table-column prop="ORG_NAME" label="门店名称" min-width="180" />
        ...
        <el-table-column prop="STOCK_AMT" label="库存金额" width="120" align="right" sortable>
...
import { getStockQuery } from '@/api/report'
...
const handleSearch = async () => {
  ...
  try {
    ...
    const res = await getStockQuery(params)
    ...
    if (res.datas && res.datas.list) {
      ...
      if (res.datas.summary) {
        summaryData.value = {
          ...
          goodsCount: res.datas.summary.goodsCount || 0
        }
      }
      ...
      res.datas.list.forEach(item => {
        ...
        shop.totalAmt += (item.STOCK_AMT || 0)
        shop.totalQty += (item.STOCK_QTY || 0)
      })
      
      shopData.value = Array.from(shopMap.values()).sort((a, b) => b.totalAmt - a.totalAmt)
      
      nextTick(() => {
        renderShopPieChart(shopData.value)
      })
      
      ElMessage.success(`查询成功，共 ${res.datas.total} 条记录，${shopMap.size} 家门店`)
    }
  } catch (error) {
